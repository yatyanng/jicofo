...
public class PubSubBridgeSelectorTest {
	...
	public static void setUpClass() throws Exception {
		System.clearProperty(BridgeSelector.BRIDGE_TO_PUBSUB_PNAME);

		System.setProperty(FocusManager.SHARED_STATS_PUBSUB_NODE_PNAME, sharedPubSubNode);

		System.setProperty(ComponentsDiscovery.ThroughPubSubDiscovery.MAX_STATS_REPORT_AGE_PNAME,
				String.valueOf(MAX_STATS_AGE));

		System.setProperty(JvbDoctor.HEALTH_CHECK_INTERVAL_PNAME, "" + HEALTH_CHECK_INT);

		osgi.init();

		meetServices = ServiceUtils.getService(osgi.bc, JitsiMeetServices.class);

		ProviderListener providerListener = new ProviderListener(FocusBundleActivator.bundleContext);

		mockProvider = (MockProtocolProvider) providerListener.obtainProvider(1000);

		xmppConnection = mockProvider.getXmppConnection();

		selector = meetServices.getBridgeSelector();

		subOpSet = mockProvider.getMockSubscriptionOpSet();
	...
	static private void createMockJvbs() throws Exception {
	...
	static private MockVideobridge createMockJvb(Jid jvbJid) throws Exception {
		MockVideobridge mockBridge = new MockVideobridge(new MockXmppConnection(jvbJid), jvbJid);

		MockCapsNode jvbNode = new MockCapsNode(jvbJid, JitsiMeetServices.VIDEOBRIDGE_FEATURES);
	...
	/**
	 * If the bridge is restarted and we have health check failed on it, but before
	 * {@link ComponentsDiscovery.ThroughPubSubDiscovery} has timed out this
	 * instance we will not re-discover it through the PubSub.
	 */
	...
	public void clearPubSubBridgeIssueTest() {
		...
		String[] features = ArrayUtils.add(JitsiMeetServices.VIDEOBRIDGE_FEATURES, String.class,
		...
		EventUtil.registerEventHandler(osgi.bc,
				new String[] { BridgeEvent.BRIDGE_UP, BridgeEvent.BRIDGE_DOWN, BridgeEvent.HEALTH_CHECK_FAILED },
				eventSpy);
		...
		verify(eventSpy, timeout(100)).handleEvent(BridgeEvent.createBridgeUp(jvb1Jid));
		...
		// send by JvbDoctor
		verify(eventSpy, timeout(HEALTH_CHECK_INT * 2)).handleEvent(BridgeEvent.createHealthFailed(jvb1Jid));

		// and after that BRIDGE_DOWN should be triggered
		// by BridgeSelector
		verify(eventSpy, timeout(100)).handleEvent(BridgeEvent.createBridgeDown(jvb1Jid));
		...
		verify(eventSpy, times(2)).handleEvent(BridgeEvent.createBridgeUp(jvb1Jid));

		// Verify events order
		InOrder eventsOrder = inOrder(eventSpy);

		eventsOrder.verify(eventSpy).handleEvent(BridgeEvent.createBridgeUp(jvb1Jid));

		eventsOrder.verify(eventSpy).handleEvent(BridgeEvent.createHealthFailed(jvb1Jid));

		eventsOrder.verify(eventSpy).handleEvent(BridgeEvent.createBridgeDown(jvb1Jid));
	...
	public void deadlockTest() throws InterruptedException, ExecutionException {
		final JitsiMeetServices meetServices = ServiceUtils.getService(osgi.bc(), JitsiMeetServices.class);

		ExecutorService executorService = Executors.newFixedThreadPool(30, new DaemonThreadFactory());
	...
	private ExtensionElement triggerJvbStats(Jid itemId, int conferenceCount) {
		...
		statsExtension.addStat(new ColibriStatsExtension.Stat(VideobridgeStatistics.CONFERENCES, "" + conferenceCount));

		subOpSet.fireSubscriptionNotification(sharedPubSubNode, itemId.toString(), statsExtension);
