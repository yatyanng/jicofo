...
import org.jitsi.jicofo.util.ConferenceRoomListener;
 ...
 *
 * @author Pawel Domas FIXME jvbDoctorTest fail randomly on ci (works locally on
 *         dev machine)
 */
public class JvbDoctorTest {
	...
	/**
	 * The role of <tt>JvbDoctor</tt> is to schedule health-check task for each new
	 * bridge discovered and advertised with {@link BridgeEvent#BRIDGE_UP} event.
	 * Then a health-check failure should trigger
	 * {@link BridgeEvent#HEALTH_CHECK_FAILED} event which after processed by the
	 * {@link BridgeSelector} should result in {@link BridgeEvent#BRIDGE_DOWN}. The
	 * last event is handled by all {@link JitsiMeetConference} which will restart
	 * if are found to be using faulty bridge. FIXME this tests fail randomly on ci
	 * (works locally on dev machine)
	 */
	public void jvbDoctorTest() throws Exception {
		Jid jvb1 = JidCreate.from("jvb1.example.com");

		FocusManager focusManager = ServiceUtils.getService(osgi.bc, FocusManager.class);

		assertNotNull(focusManager);

		MockProtocolProvider focusPps = (MockProtocolProvider) focusManager.getProtocolProvider();

		assertNotNull(focusPps);

		MockVideobridge mockBridge = new MockVideobridge(new MockXmppConnection(jvb1), jvb1);

		// Make sure that jvb advertises health-check support
		MockSetSimpleCapsOpSet mockCaps = focusPps.getMockCapsOpSet();
		MockCapsNode jvbNode = new MockCapsNode(jvb1, new String[] { DiscoveryUtil.FEATURE_HEALTH_CHECK });
		...
		verify(eventSpy, timeout(5000)).handleEvent(BridgeEvent.createBridgeUp(jvb1));
		...
		for (int i = 0; i < testConfs.length; i++) {
			testConfs[i] = TestConference.allocate(osgi.bc, serverName, JidCreate.entityBareFrom(roomName + i),
					mockBridge);
		...
		for (TestConference testConf1 : testConfs) {
			assertNotNull(focusManager.getConference(testConf1.getRoomName()));
		}

		// Here we verify that first there was HEALTH_CHECK_FAILED event
		// send by JvbDoctor
		verify(eventSpy, timeout(HEALTH_CHECK_INT + 100)).handleEvent(BridgeEvent.createHealthFailed(jvb1));
		// and after that BRIDGE_DOWN should be triggered
		// by BridgeSelector
		verify(eventSpy, timeout(100)).handleEvent(BridgeEvent.createBridgeDown(jvb1));
		...
		for (TestConference testConf : testConfs) {
			JitsiMeetConference conference = focusManager.getConference(testConf.getRoomName());
		...
		confRoomListener.await(osgi.bc(), testConfs.length, 5, TimeUnit.SECONDS);
