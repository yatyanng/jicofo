...
public class BridgeSelectorTest {
	...
	public static void setUpClass() throws Exception {
		...
		String bridgeMapping = jvb1Jid + ":" + jvb1PubSubNode + ";" + jvb2Jid + ":" + jvb2PubSubNode + ";" + jvb3Jid
				+ ":" + jvb3PubSubNode + ";";

		System.setProperty(BridgeSelector.BRIDGE_TO_PUBSUB_PNAME, bridgeMapping);
	...
	private void createMockJvbNodes(JitsiMeetServices meetServices, MockProtocolProvider protocolProvider) {
		MockSetSimpleCapsOpSet capsOpSet = protocolProvider.getMockCapsOpSet();

		MockCapsNode jvb1Node = new MockCapsNode(jvb1Jid, JitsiMeetServices.VIDEOBRIDGE_FEATURES);

		MockCapsNode jvb2Node = new MockCapsNode(jvb2Jid, JitsiMeetServices.VIDEOBRIDGE_FEATURES);

		MockCapsNode jvb3Node = new MockCapsNode(jvb3Jid, JitsiMeetServices.VIDEOBRIDGE_FEATURES);
	...
	public void selectorTest() throws InterruptedException {
		JitsiMeetServices meetServices = ServiceUtils.getService(osgi.bc, JitsiMeetServices.class);
		...
		assertEquals(jvb1Jid, selector.getBridgeForPubSubNode(jvb1PubSubNode));
		assertEquals(jvb2Jid, selector.getBridgeForPubSubNode(jvb2PubSubNode));
		assertEquals(jvb3Jid, selector.getBridgeForPubSubNode(jvb3PubSubNode));
		...
		assertTrue(workingBridges.contains(selector.selectBridge(null).getJid()));
		...
		MockSubscriptionOpSetImpl mockSubscriptions = mockProvider.getMockSubscriptionOpSet();
		...
		mockSubscriptions.fireSubscriptionNotification(jvb1PubSubNode, itemId, createJvbStats(10));
		mockSubscriptions.fireSubscriptionNotification(jvb2PubSubNode, itemId, createJvbStats(23));
		mockSubscriptions.fireSubscriptionNotification(jvb3PubSubNode, itemId, createJvbStats(0));

		assertEquals(jvb3Jid, selector.selectBridge(null).getJid());

		// Now Jvb 3 gets occupied the most
		mockSubscriptions.fireSubscriptionNotification(jvb3PubSubNode, itemId, createJvbStats(300));
		...
		mockSubscriptions.fireSubscriptionNotification(jvb1PubSubNode, itemId, createJvbStats(1));
		mockSubscriptions.fireSubscriptionNotification(jvb2PubSubNode, itemId, createJvbStats(0));
		mockSubscriptions.fireSubscriptionNotification(jvb3PubSubNode, itemId, createJvbStats(0));

		// JVB 1 should not be in front
		assertNotEquals(jvb1PubSubNode, selector.selectBridge(null).getJid());

		// JVB 2 least occupied
		mockSubscriptions.fireSubscriptionNotification(jvb1PubSubNode, itemId, createJvbStats(1));
		mockSubscriptions.fireSubscriptionNotification(jvb2PubSubNode, itemId, createJvbStats(0));
		mockSubscriptions.fireSubscriptionNotification(jvb3PubSubNode, itemId, createJvbStats(1));
	...
	private void testFailureResetThreshold(BridgeSelector selector, MockSubscriptionOpSetImpl mockSubscriptions)
			throws InterruptedException {
		Jid[] nodes = new Jid[] { jvb1Jid, jvb2Jid, jvb3Jid };
		Bridge[] states = new Bridge[] { jvb1, jvb2, jvb3 };

		String[] pubSubNodes = new String[] { jvb1PubSubNode, jvb2PubSubNode, jvb3PubSubNode };
		...
		for (int testNode = 0; testNode < nodes.length; testNode++) {
			for (int idx = 0; idx < nodes.length; idx++) {
				...
				mockSubscriptions.fireSubscriptionNotification(pubSubNodes[idx], "randomItemId",
			...
			assertNotEquals(nodes[testNode], selector.selectBridge(null).getJid());
			// Wait for faulty status reset
			Thread.sleep(150);
			// Test node should recover
			assertEquals(nodes[testNode], selector.selectBridge(null).getJid());
		}

		selector.setFailureResetThreshold(BridgeSelector.DEFAULT_FAILURE_RESET_THRESHOLD);
	}
	...
	ExtensionElement createJvbStats(int videoStreamCount) {
		...
		statsExtension
				.addStat(new ColibriStatsExtension.Stat(VideobridgeStatistics.VIDEOSTREAMS, "" + videoStreamCount));
