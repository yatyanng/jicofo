...
public class ConferenceIqProviderTest {
	@Test
	public void testParseConferenceIq() throws Exception {
		// ConferenceIq
		String iqXml = "<iq to='t' from='f' type='set'>" + "<conference xmlns='http://jitsi.org/protocol/focus'"
				+ " room='someroom@example.com' ready='true'" + ">"
				+ "<property xmlns='http://jitsi.org/protocol/focus' " + "name='name1' value='value1'/>"
				+ "<property name='name2' value='value2'/>" + "</conference>" + "</iq>";

		ConferenceIqProvider provider = new ConferenceIqProvider();
		ConferenceIq conference = IQUtils.parse(iqXml, provider);

		assertEquals("someroom@example.com", conference.getRoom().toString());
	...
	@Test
	public void testParseLoginUrlIq() throws Exception {
		...
		String authUrlIqXml = "<iq to='to1' from='from3' type='result'>"
				+ "<login-url xmlns='http://jitsi.org/protocol/focus'" + " url=\'" + encodedUrl
				+ "\' room='someroom1234@example.com' />" + "</iq>";

		LoginUrlIq authUrlIq = IQUtils.parse(authUrlIqXml, new LoginUrlIqProvider());
		...
		assertEquals("someroom1234@example.com", authUrlIq.getRoom().toString());
	}

	@Test
	public void testConferenceIqToXml() throws IOException, SAXException {
		...
		conferenceIq.addProperty(new ConferenceIq.Property("prop1", "some1"));
		conferenceIq.addProperty(new ConferenceIq.Property("name2", "xyz2"));

		assertXMLEqual(new Diff("<iq to='tojid@example.com' " + "from='fromjid@example.com' " + "id='123xyz' "
				+ "type='get'>" + "<conference " + "xmlns='http://jitsi.org/protocol/focus' "
				+ "room='testroom1234@example.com' ready='false'" + ">"
				+ "<property xmlns='http://jitsi.org/protocol/focus' name='prop1' value='some1'/>"
				+ "<property xmlns='http://jitsi.org/protocol/focus' name='name2' value='xyz2'/>" + "</conference>"
				+ "</iq>", conferenceIq.toXML().toString()), true);
	}

	@Test
	public void testLoginUrlIqToXml() throws UnsupportedEncodingException, XmppStringprepException {
		...
		assertEquals("<iq to='to657@example.com' " + "from='23from2134#@1' id='1df:234sadf' " + "type='result'>"
				+ "<login-url " + "xmlns='http://jitsi.org/protocol/focus' " + "url=\'" + encodedUrl + "\' "
				+ "room='room@sdaf.dsf.dsf'" + "/>" + "</iq>", authUrlIQ.toXML().toString());
	}
}
