 ...
 * @author Pawel Domas
 */
public class AllocThreadingTestColibriConference extends ColibriConferenceImpl {
	 ...
	 */
	private BlockingQueue<String> confCreatorQueue = new ArrayBlockingQueue<>(1);
	 ...
	 * {@link ColibriConferenceImpl.ConferenceCreationSemaphore}. Used to verify if
	 * all running threads have reached the semaphore.
	 */
	private BlockingQueue<String> createConfSemaphoreQueue = new LinkedBlockingQueue<>();

	/**
	 * Blocking queue used to put and acquire endpoints that have sent it's request
	 * packets.
	 */
	private BlockingQueue<String> requestsSentQueue = new LinkedBlockingQueue<>();
	 ...
	 * response packets. Used to verify if all threads have received their response
	 * packets.
	 */
	private BlockingQueue<String> responseReceivedQueue = new LinkedBlockingQueue<>();
	 ...
	 *
	 * @param connection XMPP connection object that wil be used by new instance.
	 */
	public AllocThreadingTestColibriConference(XmppConnection connection, EventAdmin eventAdmin) {
		super(connection, eventAdmin);
	}

	/**
	 * Sets whether or not conference creator thread should be blocked before before
	 * it manages to sent it's "create" request.
	 *
	 * @param block <tt>true</tt> to block creator thread or <tt>false</tt> to leave
	 *              it alone.
	 */
	 ...
	 *
	 * @return the name of conference creator endpoint or <tt>null</tt> if we have
	 *         failed to obtain within 5 seconds timeout.
	 *
	 * @throws InterruptedException if thread has been interrupted while waiting
	 */
	public String obtainConferenceCreator() throws InterruptedException {
		return confCreatorQueue.poll(5, TimeUnit.SECONDS);
	}

	/**
	 * Waits until all threads serving endpoints listed in <tt>endpointToEnter</tt>
	 * until they reach "conference creation semaphore"
	 *
	 * The test will fail if all threads will not end up on the semaphore within 5
	 * seconds timeout(timeout is counted since the time when the last endpoint has
	 * arrived on the semaphore).
	 *
	 * @param endpointToEnter the list of endpoint we want to be on the "conference
	 *                        creation semaphore".
	 *
	 * @throws InterruptedException if the thread has been interrupted while waiting
	 *                              for endpoints.
	 */
	public void waitAllOnCreateConfSemaphore(List<String> endpointToEnter) throws InterruptedException {
		...
		while (!endpointsCopy.isEmpty()) {
			...
			if (endpoint != null) {
				...
				fail("Endpoints have not reached " + "create conf semaphore: " + endpointsCopy);
			}
		}
	}

	public String nextOnCreateConfSemaphore(long timeoutSec) throws InterruptedException {
	...
	public String nextRequestSent(long timeoutSeconds) throws InterruptedException {
		return requestsSentQueue.poll(timeoutSeconds, TimeUnit.SECONDS);
	}

	public String nextResponseReceived(long timeoutSeconds) throws InterruptedException {
	...
	protected Stanza sendAllocRequest(String endpointId, ColibriConferenceIQ request) throws OperationFailedException {
		...
		if (responseError == null) {
			...
			response = IQ.createErrorResponse(request, XMPPError.getBuilder(responseError));
		}
	...
	/**
	 * Returns the type of error which will be returned as a response to conference
	 * create request.
	 */
	 ...
	 * create request.
	 *
	 * @param responseError the type fo the error to be returned or <tt>null</tt> to
	 *                      not interfere into the response returned by the bridge.
	 */
