...
public class MockMultiUserChat extends AbstractChatRoom implements ChatRoom2 {
	...
	/**
	 * Listeners that will be notified of changes in member status in the room such
	 * as member joined, left or being kicked or dropped.
	 */
	...
	public MockMultiUserChat(EntityBareJid roomName, ProtocolProviderService protocolProviderService) {
	...
	public void joinAs(String nickname, byte[] password) throws OperationFailedException {
		...
		try {
			...
			throw new OperationFailedException("Invalid mock room member JID",
	...
	public MockRoomMember mockJoin(String nickname) throws XmppStringprepException {
		return mockJoin(createMockRoomMember(nickname));
	}

	public MockRoomMember createMockRoomMember(String nickname) throws XmppStringprepException {
	...
	public MockRoomMember mockJoin(MockRoomMember member) {
		synchronized (members) {
			...
			fireMemberPresenceEvent(member, member, ChatRoomMemberPresenceChangeEvent.MEMBER_JOINED, null);
	...
	public void grantModerator(String address) {
		try {
			grantRole(JidCreate.entityFullFrom(address), ChatRoomMemberRole.MODERATOR);
		} catch (XmppStringprepException e) {
	...
	public void grantOwnership(String address) {
		try {
			grantRole(JidCreate.entityFullFrom(address), ChatRoomMemberRole.OWNER);
		} catch (XmppStringprepException e) {
	 ...
	 * <tt>ChatRoomMemberPresenceListener</tt>s that a ChatRoomMember has joined or
	 * left this <tt>ChatRoom</tt>.
	 *
	 ...
	 */
	private void fireMemberPresenceEvent(ChatRoomMember member, ChatRoomMember actor, String eventID,
			String eventReason) {
		ChatRoomMemberPresenceChangeEvent evt = new ChatRoomMemberPresenceChangeEvent(this, member, actor, eventID,
				eventReason);
	...
	private void fireLocalUserRoleEvent(ChatRoomMember member, ChatRoomMemberRole oldRole, boolean isInitial) {
		ChatRoomLocalUserRoleChangeEvent evt = new ChatRoomLocalUserRoleChangeEvent(this, oldRole, member.getRole(),
				isInitial);
	...
	private void fireMemberRoleEvent(ChatRoomMember member, ChatRoomMemberRole oldRole) {
		ChatRoomMemberRoleChangeEvent evt = new ChatRoomMemberRoleChangeEvent(this, member, oldRole, member.getRole());
