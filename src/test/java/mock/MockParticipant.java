...
import org.jitsi.impl.protocol.xmpp.DefaultJingleRequestHandler;
...
import mock.muc.MockRoomMember;
...
public class MockParticipant {
	 ...
	 * The logger.
	 */
	private final static Logger logger = Logger.getLogger(MockParticipant.class);
	...
	private BlockingQueue<JingleIQ> ssrcAddQueue = new LinkedBlockingQueue<>();

	private BlockingQueue<JingleIQ> ssrcRemoveQueue = new LinkedBlockingQueue<>();
	...
	public void waitForJoinThread(long timeout) throws InterruptedException {
		synchronized (joinLock) {
			if (xmppPeer == null) {
				joinLock.wait(timeout);
			}
			if (xmppPeer == null) {
				throw new RuntimeException("Failed to join the room within" + " the time limit specified: " + timeout);
			}
		}
	}
	...
	private void initContents() {
		...
		RtpDescriptionPacketExtension audioRtpDesc = new RtpDescriptionPacketExtension();
		...
		RtpDescriptionPacketExtension videoRtpDesc = new RtpDescriptionPacketExtension();
	...
	public JingleIQ[] acceptInvite(long timeout) throws InterruptedException {
		...
		// ACK invite
		IQ inviteAck = IQ.createResultIQ(invite);
	...
	private Map<String, IceUdpTransportPacketExtension> createTransportMap(JingleIQ user1Invite) {
		...
		for (ContentPacketExtension content : user1Invite.getContentList()) {
			...
			IceUdpTransportPacketExtension offerTransport = content
					.getFirstChildOfType(IceUdpTransportPacketExtension.class);

			DtlsFingerprintPacketExtension offerFingerprint = offerTransport
			...
			dtlsFingerprint.setFingerprint(
					offerFingerprint.getFingerprint().replace("A", "B").replace("1", "2").replace("C", "D"));
	...
	public Map<String, IceUdpTransportPacketExtension> generateFakeCandidates() {
		for (ContentPacketExtension content : myContents) {
			IceUdpTransportPacketExtension iceTransport = transportMap.get(content.getName());

			CandidatePacketExtension fakeCandidate = new CandidatePacketExtension();
	...
	public JingleIQ sendTransportInfo() {
		...
		for (ContentPacketExtension myContent : myContents) {
			...
			try {
				IceUdpTransportPacketExtension transportCopy = IceUdpTransportPacketExtension
		...
		JingleIQ transportInfoIq = JinglePacketFactory.createTransportInfo(myJid, remoteJid,
				jingleSession.getSessionID(), contents);
	...
	private JingleIQ generateSessionAccept(JingleIQ sessionInit,
	...
	public void processStanza(IQ packet) {
		...
		if (JingleAction.SOURCEADD.equals(action) || JingleAction.ADDSOURCE.equals(action)) {
			synchronized (sourceLock) {
				MediaSourceMap ssrcMap = MediaSourceMap.getSourcesFromContent(modifySSRcIq.getContentList());

				remoteSSRCs.add(ssrcMap);

				MediaSourceGroupMap ssrcGroupMap = MediaSourceGroupMap
						.getSourceGroupsForContents(modifySSRcIq.getContentList());
		...
		} else if (JingleAction.SOURCEREMOVE.equals(action) || JingleAction.REMOVESOURCE.equals(action)) {
			synchronized (sourceLock) {
				...
				MediaSourceGroupMap ssrcGroupsToRemove = MediaSourceGroupMap
						.getSourceGroupsForContents(modifySSRcIq.getContentList());

				remoteSSRCgroups.remove(ssrcGroupsToRemove);

				logger.info("source-remove received " + nick + " " + ssrcsToRemove);
	...
	public void switchVideoSSRCs(long[] newVideoSSRCs, boolean useSsrcGroups) {
		...
		// Send source-remove
		jingle.sendRemoveSourceIQ(toRemove, localSSRCGroups.copy(), jingleSession);
	...
	private SourceGroup getLocalSSRCGroup(String media) {
		List<SourceGroup> videoGroups = localSSRCGroups.getSourceGroupsForMedia(media);
	...
	public List<SourcePacketExtension> sourceAdd(String media, long[] newSSRCs, boolean useSsrcGroups,
			String[] ssrcsVideoTypes) {
	...
	public JingleIQ waitForAddSource(long timeout) throws InterruptedException {
		return ssrcAddQueue.poll(timeout, TimeUnit.MILLISECONDS);
	}

	public JingleIQ waitForRemoveSource(long timeout) throws InterruptedException {
		return ssrcRemoveQueue.poll(timeout, TimeUnit.MILLISECONDS);
	}

	public void waitForSSRCCondition(SSRCCondition condition, long timeout) throws InterruptedException {
		synchronized (sourceLock) {
			long start = System.currentTimeMillis();
			long end = start + timeout;

			while (!condition.checkCondition(this) && System.currentTimeMillis() < end) {
	...
	public SourcePacketExtension addLocalSSRC(String media, long ssrc, String videoType) {
