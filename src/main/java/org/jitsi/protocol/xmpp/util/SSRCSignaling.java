...
public class SSRCSignaling {
	...
	/**
	 * Copies value of "<parameter>" SSRC child element. The parameter to be copied
	 * must exist in both source and destination SSRCs.
	 * 
	 * @param dst  target <tt>SourcePacketExtension</tt> to which we want copy
	 *             parameter value.
	 * @param src  origin <tt>SourcePacketExtension</tt> from which we want to copy
	 *             parameter value.
	 * @param name the name of the parameter to copy.
	 */
	private static void copyParamAttr(SourcePacketExtension dst, SourcePacketExtension src, String name) {
	 ...
	 * {@link SourceGroupPacketExtension}s in the given {@link MediaSourceGroupMap}
	 * with the corresponding ones from the given {@link MediaSourceMap}<tt></tt>.
	 *
	 ...
	 *                               {@link SourcePacketExtension} found in the
	 *                               <tt>sources</tt> for any source in the
	 *                               <tt>groups</tt>.
	 */
	static public void copySourceParamsToGroups(MediaSourceGroupMap groups, MediaSourceMap sources)
			throws InvalidSSRCsException {
		for (String mediaType : groups.getMediaTypes()) {
			List<SourceGroup> mediaGroups = groups.getSourceGroupsForMedia(mediaType);
			List<SourceGroup> newMediaGroups = new ArrayList<>(mediaGroups.size());

			for (SourceGroup group : mediaGroups) {
				List<SourcePacketExtension> groupSources = group.getSources();
				List<SourcePacketExtension> newSources = new ArrayList<>(groupSources.size());

				for (SourcePacketExtension srcInGroup : groupSources) {
					SourcePacketExtension sourceInMedia = sources.findSource(mediaType, srcInGroup);

					if (sourceInMedia == null) {
						throw new InvalidSSRCsException(
								"Source " + srcInGroup + " not found in '" + mediaType + "' for group: " + group);
					}

					newSources.add(sourceInMedia);
				}

				newMediaGroups.add(new SourceGroup(group.getSemantics(), newSources));
			}
	 ...
	 * <tt>SourcePacketExtension</tt>.
	 *
	 * @param ssrcPe the instance of <tt>SourcePacketExtension</tt> which will be
	 *               stripped off all parameters.
	 */
	...
	/**
	 * Finds the first SSRC in the list with a valid stream ID('msid'). The
	 * 'default' stream id is not considered a valid one.
	 *
	 * @param ssrcs the list of <tt>SourcePacketExtension</tt> to be searched.
	 *
	 * @return the first <tt>SourcePacketExtension</tt> with a valid media stream id
	 *         or <tt>null</tt> if there aren't any such streams in the list.
	 */
	public static SourcePacketExtension getFirstWithMSID(List<SourcePacketExtension> ssrcs) {
	...
	private static List<SourcePacketExtension> getAllWithMSID(List<SourcePacketExtension> ssrcs) {
	 ...
	 * @return <tt>String</tt> value of the MSID attribute of given source extension
	 *         or <tt>null</tt> if it's either empty or not present.
	 */
	 ...
	 * 
	 * @param ssrc the <tt>SourcePacketExtension</tt> to be searched for parameter
	 * @param name the name of the parameter to be found
	 * @return <tt>ParameterPacketExtension</tt> instance for given <tt>name</tt> or
	 *         <tt>null</tt> if not found.
	 */
	private static ParameterPacketExtension getParam(SourcePacketExtension ssrc, String name) {
	 ...
	 * 
	 * @param ssrcPe the <tt>SourcePacketExtension</tt> instance for which we want
	 *               to find an owner.
	 * @return MUC {@link Jid} of the user who owns this source or <tt>null</tt> if
	 *         it's not owned by anyone.
	 */
	public static Jid getSSRCOwner(SourcePacketExtension ssrcPe) {
		SSRCInfoPacketExtension ssrcInfo = ssrcPe.getFirstChildOfType(SSRCInfoPacketExtension.class);
	 ...
	 * 
	 * @param ssrc <tt>SourcePacketExtension</tt> that describes the SSRC for which
	 *             we want to obtain WebRTC stream ID.
	...
	public static boolean mergeVideoIntoAudio(MediaSourceMap peerSSRCs) {
		...
		List<SourcePacketExtension> videoSSRCs = peerSSRCs.getSourcesForMedia(MediaType.VIDEO.toString());
	 ...
	 *         mapping of the SSRCs contained in given Jingle content list. An owner
	 *         comes form the {@link SSRCInfoPacketExtension} included as a child of
	 *         the {@link SourcePacketExtension}.
	 */
	public static Map<Jid, MediaSourceMap> ownerMapping(List<ContentPacketExtension> contents) {
		Map<Jid, MediaSourceMap> ownerMapping = new HashMap<>();
		for (ContentPacketExtension content : contents) {
			String media = content.getName();
			MediaSourceMap mediaSourceMap = MediaSourceMap.getSourcesFromContent(contents);

			for (SourcePacketExtension ssrc : mediaSourceMap.getSourcesForMedia(media)) {
	...
	/**
	 * From the given list of {@link SourceGroup}s select those that share the given
	 * MSID value.
	 *
	 * @param groups    a {@link List} of {@link SourceGroup} to be filtered out.
	 * @param groupMsid a {@link String} with the MSID value to be used as selector.
	 *
	 * @return a {@link List} of {@link SourceGroup}.
	 */
	public static List<SourceGroup> selectWithMsid(List<SourceGroup> groups, String groupMsid) {
