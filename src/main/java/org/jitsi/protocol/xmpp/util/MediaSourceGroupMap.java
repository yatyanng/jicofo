...
public class MediaSourceGroupMap {
	 ...
	 * Creates new instance of <tt>MediaSourceGroupMap</tt>.
	 * 
	 * @param map the map with predefined values that will be used by new instance.
	 */
	...
	public List<SimulcastGrouping> findSimulcastGroupings() {
		...
		for (SourceGroup simGroup : simGroups) {
			if (!simGroup.getSemantics().equals(SourceGroupPacketExtension.SEMANTICS_SIMULCAST)) {
				continue;
			}
			...
			SimulcastGrouping simGrouping = new SimulcastGrouping(simGroup, relatedFidGroups);
	...
	public List<SourceGroup> getRtxGroups() {
		return findSourceGroups(MediaType.VIDEO.toString(), SourceGroupPacketExtension.SEMANTICS_FID);
	}
	...
	public List<SourceGroup> getSimulcastGroups() {
		return findSourceGroups(MediaType.VIDEO.toString(), SourceGroupPacketExtension.SEMANTICS_SIMULCAST);
	}

	/**
	 * Returns the list of {@link SourceGroup} for given media type.
	 * 
	 * @param mediaType the name of media type for which list of source groups will
	 *                  be returned.
	 */
	 ...
	 * @return a {@link List} of {@link SourceGroup}
	 */
	private List<SourceGroup> findSourceGroups(String mediaType, String semantics) {
		Objects.requireNonNull(semantics, "semantics");

		List<SourceGroup> mediaGroups = groupMap.get(mediaType);

		return mediaGroups.stream().filter(mg -> semantics.equalsIgnoreCase(mg.getSemantics()))
	 ...
	 */
	public static MediaSourceGroupMap getSourceGroupsForContents(List<ContentPacketExtension> contents) {
		MediaSourceGroupMap mediaSourceGroupMap = new MediaSourceGroupMap();

		for (ContentPacketExtension content : contents) {
			List<SourceGroup> mediaGroups = mediaSourceGroupMap.getSourceGroupsForMedia(content.getName());
	...
	public void add(MediaSourceGroupMap sourceGroups) {
		sourceGroups.getMediaTypes().forEach(mediaType -> sourceGroups.getSourceGroupsForMedia(mediaType)
				.forEach(group -> addSourceGroup(mediaType, group)));
	}
	 ...
	 *         already or <tt>false</tt> otherwise. A group is considered equal when
	 *         it has the same semantics and sources stored. The order of sources
	 *         appearing in the group is important as well.
	 */
	public boolean containsGroup(String mediaType, SourceGroup toFind) {
		return getSourceGroupsForMedia(mediaType).stream().anyMatch(group -> group.equals(toFind));
	}
	...
	/**
	 * Removes source groups contained in the given <tt>MediaSourceGroupMap</tt>
	 * from this map if they exist.
	 * 
	...
	public MediaSourceGroupMap remove(MediaSourceGroupMap mapToRemove) {
		...
		for (String mediaType : mapToRemove.groupMap.keySet()) {
			...
			for (SourceGroup sourceGroupToCheck : mapToRemove.groupMap.get(mediaType)) {
	...
	public MediaSourceGroupMap copy() {
		...
		for (String mediaType : groupMap.keySet()) {
			List<SourceGroup> listToCopy = new ArrayList<>(groupMap.get(mediaType));
	...
	private String groupsToString(List<SourceGroup> sources) {
		...
		for (SourceGroup group : sources) {
			...
			str.append("SourceGroup(").append(sourceGroup.getSemantics()).append(")[ ");
	 ...
	 *         <tt>SourceGroupPacketExtension</tt> which reflects current state of
	 *         this <tt>MediaSourceGroupMap</tt>.
	 */
	public Map<String, List<SourceGroupPacketExtension>> toMap() {
		...
		for (String mediaType : groupMap.keySet()) {
			...
			List<SourceGroupPacketExtension> peGroups = new ArrayList<>(groups.size());
