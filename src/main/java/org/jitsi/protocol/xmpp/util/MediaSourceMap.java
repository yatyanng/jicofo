...
public class MediaSourceMap {
	 ...
	 */
	private final Map<String, CopyOnWriteArrayList<SourcePacketExtension>> sources;
	...
	/**
	 * Creates new instance of <tt>MediaSourceMap</tt> initialized with given map of
	 * media sources.
	 *
	 * @param sources initial map of media sources.
	 */
	private MediaSourceMap(Map<String, CopyOnWriteArrayList<SourcePacketExtension>> sources) {
	...
	public List<SourcePacketExtension> getSourcesForMedia(String mediaType) {
		return sources.computeIfAbsent(mediaType, k -> new CopyOnWriteArrayList<>());
	}
	...
	/**
	 * Adds source to this map. NOTE that duplicated sources wil be stored in the
	 * map.
	 *
	 ...
	 */
	public void addSources(String mediaType, Collection<SourcePacketExtension> newSources) {
	...
	public MediaSourceMap copyDeep() {
		Map<String, CopyOnWriteArrayList<SourcePacketExtension>> mapCopy = new HashMap<>();

		for (String mediaType : sources.keySet()) {
			CopyOnWriteArrayList<SourcePacketExtension> sourcesCopy = sources.get(mediaType).stream()
					.map(SourcePacketExtension::copy).collect(Collectors.toCollection(CopyOnWriteArrayList::new));
	 ...
	 *
	 * @return <tt>SourcePacketExtension</tt> found in this map which has the same
	 *         SSRC number as given in the <tt>ssrcValue</tt> or <tt>null</tt> if
	 *         not found.
	 */
	public SourcePacketExtension findSource(String mediaType, SourcePacketExtension sourceToFind) {
		return getSourcesForMedia(mediaType).stream().filter(source -> source.sourceEquals(sourceToFind)).findFirst()
				.orElse(null);
	}

	public SourcePacketExtension findSourceViaSsrc(String mediaType, long ssrc) {
		return getSourcesForMedia(mediaType).stream().filter(source -> source.hasSSRC() && source.getSSRC() == ssrc)
	 ...
	 *
	 * @return a {@link List} of {@link SourcePacketExtension} that matches the
	 *         given MSID.
	 */
	public List<SourcePacketExtension> findSourcesWithMsid(String mediaType, String groupMsid) {
		...
		return mediaSources.stream().filter(source -> groupMsid.equalsIgnoreCase(SSRCSignaling.getMsid(source)))
				.collect(Collectors.toCollection(LinkedList::new));
	}

	/**
	 * Looks for given source and returns type of the media for the first match
	 * found.
	 * 
	...
	public String getMediaTypeForSource(SourcePacketExtension source) {
		...
		return mediaTypes.stream().filter(mediaType -> findSource(mediaType, source) != null).findFirst().orElse(null);
	}
	...
	public SourcePacketExtension findSsrcForOwner(String mediaType, Jid owner) {
		...
		return mediaSSRCs.stream().filter(ssrc -> {
	...
	public MediaSourceMap remove(MediaSourceMap mapToRemove) {
		...
		for (String mediaType : mapToRemove.sources.keySet()) {
			...
			for (SourcePacketExtension sourceToRemove : mapToRemove.sources.get(mediaType)) {
				sourceList.stream().filter(sourceToRemove::sourceEquals).forEach(toBeRemoved::add);
			}
	...
	public boolean isEmpty() {
		return sources.keySet().stream().allMatch(mediaType -> getSourcesForMedia(mediaType).isEmpty());
	}

	/**
	 * Extracts <tt>SourcePacketExtension</tt> from given <tt>contents</tt> list and
	 * creates {@link MediaSourceMap} that reflects this contents list.
	 *
	 * @param contents the list of {@link ContentPacketExtension} from which
	 *                 <tt>SourcePacketExtension</tt> will be extracted and put into
	 *                 {@link MediaSourceMap}.
	 *
	 * @return the {@link MediaSourceMap} that describes given <tt>contents</tt>
	 *         list.
	 */
	public static MediaSourceMap getSourcesFromContent(List<ContentPacketExtension> contents) {
		Map<String, CopyOnWriteArrayList<SourcePacketExtension>> mediaSourceMap = new HashMap<>();

		for (ContentPacketExtension content : contents) {
			RtpDescriptionPacketExtension rtpDesc = content.getFirstChildOfType(RtpDescriptionPacketExtension.class);

			String mediaType = rtpDesc != null ? rtpDesc.getMedia() : content.getName();
			List<SourcePacketExtension> sourcePacketExtensions = rtpDesc != null
					? rtpDesc.getChildExtensionsOfType(SourcePacketExtension.class)
					: content.getChildExtensionsOfType(SourcePacketExtension.class);

			mediaSourceMap.put(mediaType, new CopyOnWriteArrayList<>(sourcePacketExtensions));
		}
	 ...
	 * <tt>MediaSourceMap</tt>.
	 *
	 * @return <tt>Map<String, List<SourcePacketExtension></tt> which reflects the
	 *         state of this <tt>MediaSourceMap</tt>.
	 */
