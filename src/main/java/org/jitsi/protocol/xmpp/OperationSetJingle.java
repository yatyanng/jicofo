 ...
 * @author Pawel Domas
 */
public interface OperationSetJingle extends OperationSet {
	/**
	 * Initiates a Jingle session by sending the provided {@code session-initiate}
	 * IQ. Blocks until a response is received or until a timeout is reached.
	 *
	 * @param jingleIQ       the IQ to send.
	 * @param requestHandler <tt>JingleRequestHandler</tt> that will be bound to new
	 *                       Jingle session instance.
	 *
	 * @return {@code true} if a response of type {@code result} is received before
	 *         the timeout.
	 *
	 * @throws OperationFailedException with
	 *                                  {@link OperationFailedException#PROVIDER_NOT_REGISTERED}
	 *                                  if the operation fails, because the XMPP
	 *                                  connection is broken.
	 */
	boolean initiateSession(JingleIQ jingleIQ, JingleRequestHandler requestHandler) throws OperationFailedException;

	/**
	 * Creates a {@code session-initiate} IQ for a specific address and adds a list
	 * of {@link ContentPacketExtension} to it.
	 *
	 ...
	 */
	JingleIQ createSessionInitiate(Jid address, List<ContentPacketExtension> contents);

	/**
	 * Sends a 'transport-replace' IQ to the client. Blocks waiting for a response
	 * and returns {@code true} if a response with type {@code result} is received
	 * before a certain timeout.
	 *
	 ...
	 *                                  connection is broken.
	 */
	boolean replaceTransport(JingleIQ jingleIQ, JingleSession session) throws OperationFailedException;
	 ...
	 * @return the IQ which was created.
	 */
	JingleIQ createTransportReplace(JingleSession session, List<ContentPacketExtension> contents);
	 ...
	 */
	void sendAddSourceIQ(MediaSourceMap ssrcMap, MediaSourceGroupMap ssrcGroupMap, JingleSession session);
	 ...
	 * @param session      the <tt>JingleSession</tt> used to send the notification.
	 */
	void sendRemoveSourceIQ(MediaSourceMap ssrcMap, MediaSourceGroupMap ssrcGroupMap, JingleSession session);
	 ...
	 *                termination.
	 * @param message optional text message providing more details about the reason
	 ...
	 * 
	 * @param requestHandler <tt>JingleRequestHandler</tt> instance for which all
	 *                       active JingleSessions shall be terminated.
	 */
