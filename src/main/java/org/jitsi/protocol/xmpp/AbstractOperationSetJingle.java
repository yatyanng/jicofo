 ...
 * @author Pawel Domas
 */
public abstract class AbstractOperationSetJingle extends AbstractIqRequestHandler implements OperationSetJingle {
	/**
	 * The {@code Logger} used by the class {@code AbstractOperationSetJingle} and
	 * its instances to print debug-related information.
	 */
	 ...
	 * @return Jingle session for given session identifier or <tt>null</tt> if no
	 *         such session exists.
	 */
	 ...
	 *                   offer
	 * @param startMuted an array where the first value stands for "start with audio
	 *                   muted" and the seconds one for "start video muted"
	 *
	 * @return New instance of <tt>JingleIQ</tt> filled up with the details provided
	 *         as parameters.
	 */
	private JingleIQ createInviteIQ(JingleAction action, String sessionId, boolean useBundle, Jid address,
			List<ContentPacketExtension> contents, boolean[] startMuted) {
	...
	/**
	 * Determines whether a specific {@link JingleSession} has been accepted by the
	 * client judging by a specific {@code reply} {@link IQ} (received in reply to
	 * an invite IQ sent withing the specified {@code JingleSession}).
	 *
	 * @param session <tt>JingleSession</tt> instance for which we're evaluating the
	 *                response value.
	...
	private boolean wasInviteAccepted(JingleSession session, IQ reply) {
		if (reply == null) {
			...
			if (session.isAccepted()) {
				...
				logger.error("Timeout waiting for RESULT response to " + "'session-initiate' request from "
						+ session.getAddress());
				return false;
			}
			...
			logger.error(
					"Failed to send 'session-initiate' to " + session.getAddress() + ", error: " + reply.getError());
			return false;
		}
	}
	...
	protected IQ processJingleIQ(JingleIQ iq) {
		...
		case SESSION_TERMINATE:
			ReasonPacketExtension reason = iq.getReason();
			error = requestHandler.onSessionTerminate(session, reason);
			break;
	...
	/**
	 * Sends 'source-add' notification to the peer of given <tt>JingleSession</tt>.
	 *
	 * @param ssrcs        the map of media SSRCs that will be included in the
	 *                     notification.
	 * @param ssrcGroupMap the map of media SSRC groups that will be included in the
	 *                     notification.
	 * @param session      the <tt>JingleSession</tt> used to send the notification.
	 */
	@Override
	public void sendAddSourceIQ(MediaSourceMap ssrcs, MediaSourceGroupMap ssrcGroupMap, JingleSession session) {
		...
		if (ssrcGroupMap != null) {
			for (String media : ssrcGroupMap.getMediaTypes()) {
				...
				for (SourceGroup sourceGroup : ssrcGroupMap.getSourceGroupsForMedia(media)) {
					try {
		...
		logger.info("Notify add SSRC " + session.getAddress() + " SID: " + session.getSessionID() + " " + ssrcs + " "
				+ ssrcGroupMap);
	 ...
	 */
	@Override
	public void sendRemoveSourceIQ(MediaSourceMap ssrcs, MediaSourceGroupMap ssrcGroupMap, JingleSession session) {
		...
		logger.info("Notify remove SSRC " + session.getAddress() + " SID: " + session.getSessionID() + " " + ssrcs + " "
				+ ssrcGroupMap);
	 ...
	 * @param reason  one of {@link Reason} enum that indicates why the session is
	 *                being ended or <tt>null</tt> to omit. {@inheritDoc}
	 */
	@Override
	public void terminateSession(JingleSession session, Reason reason, String message) {
		...
		JingleIQ terminate = JinglePacketFactory.createSessionTerminate(getOurJID(), session.getAddress(),
				session.getSessionID(), reason, message);
