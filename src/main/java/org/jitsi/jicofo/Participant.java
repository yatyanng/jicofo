...
public class Participant extends AbstractParticipant {
	 ...
	 * returns <tt>false</tt>.
	 */
	private Map<String, IceUdpTransportPacketExtension> transportMap = new HashMap<>();
	 ...
	 */
	public Participant(JitsiMeetConference conference, XmppChatMember roomMember, int maxSourceCount) {
	...
	/**
	 * Returns {@link JingleSession} established with this conference participant or
	 * <tt>null</tt> if there is no session yet.
	 */
	...
	/**
	 * Returns {@link XmppChatMember} that represents this participant in conference
	...
	/**
	 * Returns <tt>true</tt> if this participant supports RTP bundle and RTCP mux.
	 */
	 ...
	 * situations where we need to know whether given peer is a human or not. For
	 * example when we close browser window and only SIP gateway stays we should
	 * destroy the conference and close SIP connection.
	 * 
	...
	public void setSupportedFeatures(List<String> supportedFeatures) {
		this.supportedFeatures = Objects.requireNonNull(supportedFeatures, "supportedFeatures");
	}
	...
	/**
	 * Return a <tt>Boolean</tt> which informs about this participant's video muted
	 * status. The <tt>null</tt> value stands for 'unknown'/not signalled,
	...
	/**
	 * Extracts and stores transport information from given map of Jingle content.
	 * Depending on the {@link #hasBundleSupport()} either 'bundle' or 'non-bundle'
	 * transport information will be stored. If we already have the transport
	 * information it will be merged into the currently stored one with
	 * {@link TransportSignaling#mergeTransportExtension}.
	 *
	 * @param contents the list of <tt>ContentPacketExtension</tt> from one of
	 *                 jingle message which can potentially contain transport info
	 *                 like 'session-accept', 'transport-info', 'transport-accept'
	 *                 etc.
	 */
	public void addTransportFromJingle(List<ContentPacketExtension> contents) {
		if (hasBundleSupport()) {
			...
			for (ContentPacketExtension cpe : contents) {
				IceUdpTransportPacketExtension contentTransport = cpe
			...
			if (transport == null) {
				logger.error(
						"No valid transport supplied in transport-update from " + getChatMember().getContactAddress());
				return;
			}
			...
			for (ContentPacketExtension cpe : contents) {
				IceUdpTransportPacketExtension srcTransport = cpe
						.getFirstChildOfType(IceUdpTransportPacketExtension.class);

				if (srcTransport != null) {
					String contentName = cpe.getName().toLowerCase();
					IceUdpTransportPacketExtension dstTransport = transportMap.get(contentName);

					if (dstTransport == null) {
						transportMap.put(contentName, srcTransport);
					} else {
						TransportSignaling.mergeTransportExtension(dstTransport, srcTransport);
					}
				}
			}
		}
	...
	/**
	 * Returns 'bundled' transport information stored for this <tt>Participant</tt>.
	 * 
	 ...
	 * <tt>Participant</tt>.
	 *
	 * @return a map of <tt>IceUdpTransportPacketExtension</tt> to Colibri content
	 *         name which describes 'non-bundled' transport of this participant or
	 *         <tt>null</tt> either if it's not available yet or if 'bundled'
	...
	/**
	 * Clears any ICE transport information currently stored for this participant.
	 */
	...
	public void claimSources(MediaSourceMap sourceMap) {
		...
		sourceMap.getMediaTypes().forEach(mediaType -> sourceMap.getSourcesForMedia(mediaType)
				.forEach(source -> SSRCSignaling.setSSRCOwner(source, roomJid)));
	}
