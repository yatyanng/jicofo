 ...
 * @author Pawel Domas
 */
public class JvbDoctor extends EventHandlerActivator implements RegistrationStateChangeListener {
	/**
	 * The logger.
	 */
	 ...
	 * intervals.
	 */
	public static final String HEALTH_CHECK_INTERVAL_PNAME = "org.jitsi.jicofo.HEALTH_CHECK_INTERVAL";

	/**
	 * The name of the configuration property used to configure 2nd chance delay.
	 * This is how long we will wait to retry the health check after 1st timeout.
	 */
	 ...
	 */
	private static final String[] HEALTH_CHECK_FEATURES = new String[] { DiscoveryUtil.FEATURE_HEALTH_CHECK };
	...
	/**
	 * 2nd chance delay which tells how long we will wait to retry the health check
	 * after 1st attempt has timed out.
	 */
	...
	/**
	 * <tt>ScheduledExecutorService</tt> reference used to schedule periodic health
	 * check tasks.
	 */
	...
	/**
	 * Capabilities operation set used to detect health check support on the bridge.
	 */
	 ...
	 */
	@Override
	synchronized public void start(BundleContext bundleContext) throws Exception {
		if (this.osgiBc != null) {
			throw new IllegalStateException("Started already?");
		}

		healthCheckInterval = FocusBundleActivator.getConfigService().getLong(HEALTH_CHECK_INTERVAL_PNAME,
		...
		secondChanceDelay = FocusBundleActivator.getConfigService().getLong(SECOND_CHANCE_DELAY_PNAME,
		...
		this.executorServiceRef = new OSGIServiceRef<>(osgiBc, ScheduledExecutorService.class);

		// We assume that in Jicofo there is only one XMPP provider running at a
		// time.
		protocolProvider = ServiceUtils.getService(osgiBc, ProtocolProviderService.class);
		...
		if (!ProtocolNames.JABBER.equals(protocolProvider.getProtocolName())) {
			throw new IllegalArgumentException("ProtocolProvider is not an XMPP one");
		}

		protocolProvider.addRegistrationStateChangeListener(this);
		connection = Objects
				.requireNonNull(protocolProvider.getOperationSet(OperationSetDirectSmackXmpp.class), "xmppOpSet")
				.getXmppConnection();

		capsOpSet = protocolProvider.getOperationSet(OperationSetSimpleCaps.class);
	 ...
	 */
	@Override
	synchronized public void stop(BundleContext bundleContext) throws Exception {
		if (this.osgiBc == null)
			return;
	...
	private void addBridge(Jid bridgeJid) {
		...
		if (executorService == null) {
			throw new IllegalStateException("No ScheduledExecutorService running!");
		}

		ScheduledFuture healthTask = executorService.scheduleAtFixedRate(new HealthCheckTask(bridgeJid),
	...
	private void notifyHealthCheckFailed(Jid bridgeJid, XMPPError error) {
		...
		if (eventAdmin == null) {
			logger.error("Unable to trigger health-check failed event: " + "no EventAdmin service found!");
			return;
		}

		logger.warn("Health check failed on: " + bridgeJid + " error: " + (error != null ? error.toXML() : "timeout"));

		eventAdmin.postEvent(BridgeEvent.createHealthFailed(bridgeJid));
	}

	/**
	 * When the xmpp protocol provider got registered, its maybe reconnection we
	 * need to get the connection. It can happen that on startup the initial
	 * obtaining the connection returns null and we get it later when the provider
	 * got actually registered.
	 *
	...
	private class HealthCheckTask implements Runnable {
		...
		/**
		 * Indicates whether or not the bridge has health-check support. If set to
		 * <tt>null</tt> it means that we don't know that yet (there was no successful
		 * disco-info exchange so far).
		 */
		...
		private void verifyHealthCheckSupport() {
			if (hasHealthCheckSupport == null) {
				...
				if (jvbFeatures != null) {
					hasHealthCheckSupport = DiscoveryUtil.checkFeatureSupport(HEALTH_CHECK_FEATURES, jvbFeatures);
					if (!hasHealthCheckSupport) {
						logger.warn(bridgeJid + " does not support health checks!");
					}
				} else {
					logger.warn("Failed to check for health check support on " + bridgeJid);
				}
			}
		}
		 ...
		 * 
		 * @throws OperationFailedException when XMPP got disconnected - the task should
		 *                                  terminate.
		 */
		private void doHealthCheck() throws OperationFailedException {
			...
			if (response == null && secondChanceDelay > 0) {
				try {
					...
					logger.warn(bridgeJid + " health-check timed out," + " but will give it another try after: "
							+ secondChanceDelay);
