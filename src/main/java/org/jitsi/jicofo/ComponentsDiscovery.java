 ...
 * @author Pawel Domas
 */
public class ComponentsDiscovery implements RegistrationStateChangeListener {
	/**
	 * The logger
	 */
	...
	/**
	 * The name of configuration property which specifies how often XMPP components
	 * re-discovery will be performed. Time interval in millis.
	 */
	private final static String REDISCOVERY_INTERVAL_PNAME = "org.jitsi.jicofo.SERVICE_REDISCOVERY_INTERVAL";

	/**
	 * {@link JitsiMeetServices} which is notified about new components discovered
	 * or when one of currently running goes offline.
	 */
	 ...
	 *
	 * @param meetServices {@link JitsiMeetServices} instance which will be notified
	 *                     about XMPP components available on XMPP domain.
	 */
	 ...
	 *
	 * @param xmppDomain              server address/main service XMPP xmppDomain
	 *                                that hosts the conference system.
	 * @param statsPubSubNode         the name of PubSub node where video bridges
	 *                                are publishing their stats. It will be used to
	 *                                discover videobridges automatically based on
	 *                                item's IDs(each bridges used it's JID as item
	 *                                ID).
	 * @param protocolProviderHandler protocol provider handler that provides XMPP
	 *                                connection
	 * @throws java.lang.IllegalStateException if started already.
	 */
	public void start(DomainBareJid xmppDomain, String statsPubSubNode,
	...
	private void scheduleRediscovery() {
		long interval = FocusBundleActivator.getConfigService().getLong(REDISCOVERY_INTERVAL_PNAME,
				DEFAULT_REDISCOVERY_INT);
		...
		if (pubSubBridgeDiscovery == null && !StringUtils.isNullOrEmpty(statsPubSubNode)) {
			OperationSetSubscription subOpSet = protocolProviderHandler.getOperationSet(OperationSetSubscription.class);

			pubSubBridgeDiscovery = new ThroughPubSubDiscovery(subOpSet, capsOpSet,
					FocusBundleActivator.getSharedThreadPool());

			pubSubBridgeDiscovery.start(FocusBundleActivator.bundleContext);
		}
	}
	...
	public void discoverServices() {
		...
		for (Jid node : nodes) {
			...
			if (!itemMap.containsKey(node)) {
				...
				logger.info("New component discovered: " + node + ", " + version);
	...
	/**
	 * Each bridge is supposed to use it's JID as item ID and this class discovers
	 * videobridges by listening to PubSub stats notifications.
	 */
	class ThroughPubSubDiscovery implements SubscriptionListener, EventHandler {
		/**
		 * The name of configuration property used to configure max bridge stats age.
		 */
		public static final String MAX_STATS_REPORT_AGE_PNAME = "org.jitsi.jicofo.MAX_STATS_REPORT_AGE";

		/**
		 * 15 seconds by default. If bridge does not publish stats for longer then it is
		 * considered offline.
		 */
		...
		/**
		 * Maps bridge JID to last received stats timestamp. Used to expire bridge which
		 * do not send stats for too long. TODO: move to BridgeSelector?
		 */
		private final Map<Jid, Long> bridgesMap = new HashMap<>();

		/**
		 * <tt>ScheduledExecutorService</tt> used to run cyclic task of bridge timestamp
		 * validation.
		 */
		 ...
		 *                          cyclic task.
		 */
		public ThroughPubSubDiscovery(OperationSetSubscription subscriptionOpSet, OperationSetSimpleCaps capsOpSet,
		...
		synchronized void start(BundleContext osgiCtx) {
			logger.info("Bridges will be discovered through" + " PubSub stats on node: " + statsPubSubNode);

			this.MAX_STATS_REPORT_AGE = FocusBundleActivator.getConfigService().getLong(MAX_STATS_REPORT_AGE_PNAME,
					MAX_STATS_REPORT_AGE);

			if (MAX_STATS_REPORT_AGE <= 0)
				throw new IllegalArgumentException("MAX STATS AGE: " + MAX_STATS_REPORT_AGE);
			...
			this.expireTask = executor.scheduleAtFixedRate(new Runnable() {
				@Override
				public void run() {
					try {
						validateBridges();
			...
			}, MAX_STATS_REPORT_AGE / 2, MAX_STATS_REPORT_AGE / 2, TimeUnit.MILLISECONDS);

			evtHandlerReg = EventUtil.registerEventHandler(osgiCtx, new String[] { BridgeEvent.BRIDGE_DOWN }, this);
		}
		...
		synchronized void stop() {
			...
			Iterator<Map.Entry<Jid, Long>> bridges = bridgesMap.entrySet().iterator();
		...
		synchronized void validateBridges() {
			...
			while (bridges.hasNext()) {
				Map.Entry<Jid, Long> bridge = bridges.next();
				if (System.currentTimeMillis() - bridge.getValue() > MAX_STATS_REPORT_AGE) {
					Jid bridgeJid = bridge.getKey();

					logger.info("No stats seen from " + bridgeJid + " for too long");
		...
		private void verifyJvbJid(Jid bridgeJid) {
			...
			if (jidFeatures == null) {
				logger.warn("Failed to discover features for: " + bridgeJid);
				return;
			}
			...
			if (JitsiMeetServices.isJitsiVideobridge(jidFeatures)) {
				...
				Version jvbVersion = DiscoveryUtil.discoverVersion(connection, bridgeJid, jidFeatures);
		...
		@Override
		public void onSubscriptionUpdate(String node, String itemId, ExtensionElement payload) {
			Jid bridgeId;
			try {
			...
			synchronized (this) {
				...
				if ("service-unavailable".equals(payload.getElementName())) {
					logger.info("Service unavailable through PubSub for " + bridgeId);
