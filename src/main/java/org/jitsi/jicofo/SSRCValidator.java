...
import org.jitsi.protocol.xmpp.util.SimulcastGrouping;
...
public class SSRCValidator {
	 ...
	 */
	private final static Logger classLogger = Logger.getLogger(SSRCValidator.class);
	...
	/**
	 * Each validation happens when a participant sends new sources and one instance
	 * of {@link SSRCValidator} is only good for one such validation. This field
	 * stores participant's endpoint ID used for printing log messages.
	 */
	private final String endpointId;

	/**
	 * The source map obtained from the participant which reflects current source
	 * status. It's a clone and modifications done here do not affect the version
	 * held by {@link Participant}.
	 */
	 ...
	 */
	static private List<SourceGroup> getIndependentFidGroups(List<SimulcastGrouping> simGroupings,
		...
		return fidGroups.stream().filter(
				fidGroup -> simGroupings.stream().noneMatch(simGroup -> simGroup.belongsToSimulcastGrouping(fidGroup)))
	 ...
	 *
	 * @param independentFidGroups the list of all independent FID groups (that do
	 *                             not belong to any other higher level grouping).
	 *
	 * @throws InvalidSSRCsException in case of MSID conflict
	 */
	static private void verifyNoMsidConflictsAcrossFidGroups(List<SourceGroup> independentFidGroups)
		...
		for (SourceGroup fidGroup : independentFidGroups) {
			...
			List<SourceGroup> withTheMsid = SSRCSignaling.selectWithMsid(independentFidGroups, fidGroupMsid);

			for (SourceGroup conflictingGroup : withTheMsid) {
				if (conflictingGroup != fidGroup) {
					throw new InvalidSSRCsException("MSID conflict across FID groups: " + fidGroupMsid + ", "
							+ conflictingGroup + " conflicts with group " + fidGroup);
				}
			}
		}
	}
	 ...
	 * @param groupedSources the map holding all sources which belong to any group
	 *                       for all media types.
	 ...
	 * @throws InvalidSSRCsException in case of MSID conflict
	 */
	static private void verifyNoMsidConflictsAcrossSimGroupings(String mediaType, MediaSourceMap groupedSources,
			List<SimulcastGrouping> simGroupings) throws InvalidSSRCsException {
		for (SimulcastGrouping simGrouping : simGroupings) {
			...
			if (simGrouping.isUsingRidSignaling()) {
				...
				throw new InvalidSSRCsException("No MSID in simulcast group: " + simGrouping);
			}

			List<SourcePacketExtension> sourcesWithTheMsid = groupedSources.findSourcesWithMsid(mediaType,
					simulcastMsid);

			for (SourcePacketExtension src : sourcesWithTheMsid) {
				if (!simGrouping.belongsToSimulcastGrouping(src)) {
					throw new InvalidSSRCsException("MSID conflict across SIM groups: " + simulcastMsid + ", " + src
	 ...
	 *                         type can be stored at a time by each conference
	 *                         participant.
	 * @param logLevelDelegate a <tt>Logger</tt> which will be used as the logging
	 *                         level delegate.
	 */
	public SSRCValidator(String endpointId, MediaSourceMap sources, MediaSourceGroupMap sourceGroups,
			int maxSourceCount, Logger logLevelDelegate) {
	...
	/**
	 * Checks how many sources are current in the conference for given participant.
	 *
	 ...
	 * @return how many sources are currently in the conference source map for given
	 *         media type and owner's JID.
	 */
	private long getSourceCountForOwner(Jid owner, String mediaType) {
		return this.sources.getSourcesForMedia(mediaType).stream()
				.filter(source -> Objects.equals(SSRCSignaling.getSSRCOwner(source), owner)).count();

	}

	/**
	 * Makes an attempt to add given sources and source groups to the current state.
	 * It checks some constraints that prevent from injecting invalid description
	 * into the conference: 1. Allow SSRC value between 1 and 0xFFFFFFFF (note that
	 * 0 is a valid value, but it breaks WebRTC stack in Chrome, so not allowed
	 * here) 2. Does not allow the same source to appear more than once per media
	 * type 3. Truncates sources above the limit (configured in the constructor) 4.
	 * Filters out SSRC parameters other than 'cname' and 'msid' 5. Drop empty
	 * source groups 6. Skips duplicated groups (the same semantics and contained
	 * sources) 7. Looks for MSID conflicts between SSRCs which do not belong to the
	 * same group 8. Makes sure that sources described by groups exist in media
	 * description
	 *
	 ...
	 *         <tt>MediaSourceGroupMap</tt> with <tt>SourceGroup</tt>s accepted by
	 *         this validator instance.
	 *
	 * @throws InvalidSSRCsException if a critical problem has been found with the
	 *                               new sources/groups which would probably result
	 *                               in "setRemoteDescription" error on the client.
	 */
	public Object[] tryAddSourcesAndGroups(MediaSourceMap newSources, MediaSourceGroupMap newGroups)
			throws InvalidSSRCsException {
		MediaSourceMap acceptedSources = new MediaSourceMap();
		for (String mediaType : newSources.getMediaTypes()) {
			List<SourcePacketExtension> mediaSources = newSources.getSourcesForMedia(mediaType);
			...
			for (SourcePacketExtension source : mediaSources) {
				if (!source.hasSSRC() && !source.hasRid()) {
					...
					throw new InvalidSSRCsException("Source with no value was passed" + " (parsed from negative ?)");
					...
					if (ssrcValue <= 0L || ssrcValue > 0xFFFFFFFFL) {
						throw new InvalidSSRCsException("Illegal SSRC value: " + ssrcValue);
					}
				}

				// Check for duplicates
				String conflictingMediaType = sources.getMediaTypeForSource(source);
				if (conflictingMediaType != null) {
					throw new InvalidSSRCsException(
							"Source " + source.toString() + " is in " + conflictingMediaType + " already");
				}
				...
				if (sourceCount >= maxSourceCount) {
					logger.error("Too many sources signalled by " + endpointId + " - dropping: " + source.toString());
		...
		for (String mediaType : newGroups.getMediaTypes()) {
			for (SourceGroup groupToAdd : newGroups.getSourceGroupsForMedia(mediaType)) {
				...
				if (sourceGroups.containsGroup(mediaType, groupToAdd)) {
					logger.warn(endpointId + " is trying to add an existing group :" + groupToAdd);
				} else {
	...
	private void filterOutParams(SourcePacketExtension copy) {
		Iterator<? extends ExtensionElement> params = copy.getChildExtensions().iterator();
		while (params.hasNext()) {
			...
			if (ext instanceof ParameterPacketExtension) {
				...
				if (!"cname".equalsIgnoreCase(ppe.getName()) && !"msid".equalsIgnoreCase(ppe.getName())) {
					params.remove();
				}
			}
		}
	...
	private void validateStreams() throws InvalidSSRCsException {
		...
		for (String mediaType : sourceGroups.getMediaTypes()) {
			List<SourceGroup> mediaGroups = sourceGroups.getSourceGroupsForMedia(mediaType);
				...
				for (SourcePacketExtension source : groupSources) {
					if (source.hasSSRC()) {
						...
						if (StringUtils.isNullOrEmpty(groupMSID)) {
							throw new InvalidSSRCsException("Grouped " + source + " has no 'msid'");
						}
						// Verify if MSID is the same across all SSRCs which
						// belong to the same group
						else if (!groupMSID.equals(msid)) {
							throw new InvalidSSRCsException("MSID mismatch detected in group " + group);
						}
					}
		...
		for (String mediaType : sourceGroups.getMediaTypes()) {
			...
			List<SimulcastGrouping> simGroupings = sourceGroups.findSimulcastGroupings();

			// Check if this SIM group's MSID does not appear in any other
			// simulcast grouping
			verifyNoMsidConflictsAcrossSimGroupings(mediaType, groupedSources, simGroupings);
			...
			List<SourceGroup> independentFidGroups = getIndependentFidGroups(simGroupings, fidGroups);
		...
		for (String mediaType : notGroupedSSRCs.getMediaTypes()) {
			Map<String, SourcePacketExtension> streamMap = new HashMap<>();

			List<SourcePacketExtension> mediaSSRCs = notGroupedSSRCs.getSourcesForMedia(mediaType);
			for (SourcePacketExtension ssrc : mediaSSRCs) {
				String msid = SSRCSignaling.getMsid(ssrc);
				if (msid != null) {
					SourcePacketExtension conflictingSSRC = streamMap.get(msid);
					if (conflictingSSRC != null) {
						throw new InvalidSSRCsException("Not grouped SSRC " + ssrc.getSSRC() + " has conflicting MSID '"
								+ msid + "' with " + conflictingSSRC.getSSRC());
					} else {
