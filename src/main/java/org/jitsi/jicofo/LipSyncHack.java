...
public class LipSyncHack implements OperationSetJingle {
	 ...
	 */
	static private final Logger classLogger = Logger.getLogger(LipSyncHack.class);
	 ...
	 *
	 * @param conference parent <tt>JitsiMeetConference</tt> for which this instance
	 *                   will be doing lip-sync hack.
	 * @param jingleImpl the Jingle operations set that will be wrapped in order to
	 *                   modify some of the Jingle requests.
	 */
	public LipSyncHack(JitsiMeetConference conference, OperationSetJingle jingleImpl) {
	...
	/**
	 * Decides whether or not it's ok to merge streams sent from one participant to
	 * another.
	 *
	 * @param participantJid the MUC JID of the participant to whom we're going to
	 *                       send stream info.
	 ...
	 */
	private boolean isOkToMergeParticipantAV(Jid participantJid, Jid ownerJid) {
		...
		logger.debug(String.format("Lips-sync From %s to %s, lip-sync: %s", ownerJid, participantJid, supportsLipSync));
	...
	private void doMerge(Jid participant, Jid owner, MediaSourceMap ssrcs) {
		...
		String logMsg = (merged ? "Merging" : "Not merging") + " A/V streams from " + owner + " to " + participant;
	 ...
	 * streams description for the whole conference which are about to be sent to
	 * one of the conference participants. It will do audio and video stream merging
	 * for the purpose of enabling lip-sync functionality on the client.
	 *
	 ...
	 *                 will be sent.
	 */
	private void processAllParticipantsSSRCs(List<ContentPacketExtension> contents, Jid mucJid) {
	 ...
	 * corresponding audio streams. A corresponding stream is the one that belongs
	 * to the same participant which we figure out by checking 'owner' included
	 * {@link SSRCInfoPacketExtension}. Once the processing is done the job is
	 * passed to the underlying Jingle operation set to send the notification.
	 *
	 * {@inheritDoc}
	 */
	@Override
	public boolean initiateSession(JingleIQ jingleIQ, JingleRequestHandler requestHandler)
	...
	/**
	 * We're doing the same thing as in {@link #initiateSession}. So go there for
	 * more docs.
	 *
	 * {@inheritDoc}
	 */
	@Override
	public boolean replaceTransport(JingleIQ jingleIQ, JingleSession session) throws OperationFailedException {
		processAllParticipantsSSRCs(jingleIQ.getContentList(), session.getAddress());
	 ...
	 */
	@Override
	public JingleIQ createSessionInitiate(Jid address, List<ContentPacketExtension> contents) {
		return jingleImpl.createSessionInitiate(address, contents);
	}

	/**
	 * We'll attempt to merge any video into the corresponding audio stream (figured
	 * out by stream 'owner'). If given notification contains only video SSRC we'll
	 * look up global conference state held by {@link JitsiMeetConference} in order
	 * to find the matching audio stream.
	 *
	 * After the processing the job is passed to the underlying Jingle operation set
	 * to send the request.
	 *
	 * {@inheritDoc}
	 */
	@Override
	public void sendAddSourceIQ(MediaSourceMap ssrcMap, MediaSourceGroupMap ssrcGroupMap, JingleSession session) {
		Jid mucJid = session.getAddress();
		// If this is source add for video only then add audio for merge process
		for (SourcePacketExtension videoSsrc : ssrcMap.getSourcesForMedia("video")) {
			Jid owner = SSRCSignaling.getSSRCOwner(videoSsrc);
			SourcePacketExtension audioSsrc = ssrcMap.findSsrcForOwner("audio", owner);
			if (audioSsrc == null) {
				...
				List<SourcePacketExtension> audioSSRCs = allOwnersSSRCs.getSourcesForMedia("audio");
			...
			if (audioSsrc != null) {
				...
				logger.warn("No corresponding audio found for: " + owner + " 'source-add' to: " + mucJid);
			}
		}
	 ...
	 * just the SSRC value. The client should remove all lines corresponding to the
	 * SSRC. Otherwise we would have to keep the track of stuff we send to each
	 * participant, as it will get out of sync with global conference state held in
	 * {@link JitsiMeetConference} after a stream is merged.
	 *
	 * {@inheritDoc}
	 */
	@Override
	public void sendRemoveSourceIQ(MediaSourceMap ssrcMap, MediaSourceGroupMap ssrcGroupMap, JingleSession session) {
	 ...
	 */
	@Override
	public void terminateSession(JingleSession session, Reason reason, String msg) {
