...
public abstract class AbstractChannelAllocator implements Runnable {
	 ...
	 */
	private final static Logger classLogger = Logger.getLogger(AbstractChannelAllocator.class);
	...
	/**
	 * The {@link JitsiMeetConferenceImpl} into which a participant will be invited.
	 */
	...
	/**
	 * A flag which indicates whether channel allocation is canceled. Raising this
	 * makes the allocation thread discontinue the allocation process and return.
	 */
	protected volatile boolean canceled = false;

	/**
	 * First argument stands for "start audio muted" and the second one for "start
	 * video muted". The information is included as a custom extension in
	 * 'session-initiate' sent to the user.
	 */
	protected final boolean[] startMuted;

	/**
	 * Indicates whether or not this task will be doing a "re-invite". It means that
	 * we're going to replace a previous conference which has failed. Channels are
	 * allocated on new JVB and peer is re-invited with 'transport-replace' Jingle
	 * action as opposed to 'session-initiate' in regular invite.
	 */
	 ...
	 * {@link JitsiMeetConferenceImpl} (using a specific jitsi-videobridge instance
	 * specified by a
	 ...
	 *                       {@link org.jitsi.jicofo.JitsiMeetConferenceImpl.BridgeSession}
	 *                       which identifies the jitsi-videobridge instance on
	 ...
	 *                       value stands for "start audio muted" and the second one
	 *                       for "video muted". This is to be included in client's
	 *                       offer.
	 * @param reInvite       whether to send an initial offer (session-initiate) or
	 *                       a an updated offer (transport-replace).
	 */
	protected AbstractChannelAllocator(JitsiMeetConferenceImpl meetConference,
			JitsiMeetConferenceImpl.BridgeSession bridgeSession, AbstractParticipant participant, boolean[] startMuted,
			boolean reInvite) {
	...
	private void doRun() throws OperationFailedException {
	 ...
	 *
	 * @return a {@link ColibriConferenceIQ} which describes the allocated channels,
	 *         or {@code null}.
	 */
		...
		while (!bridgeSession.colibriConference.isDisposed() && !bridgeSession.hasFailed) {
			try {
				...
				if (bridgeSession.colibriConference.hasJustAllocated()) {
					meetConference.onColibriConferenceAllocated(bridgeSession.colibriConference, jvb);
				}
				return colibriChannels;
			} catch (OperationFailedException exc) {
				logger.error("Failed to allocate channels using bridge: " + jvb, exc);
				...
				if (OperationFailedException.ILLEGAL_ARGUMENT != exc.getErrorCode()) {
					bridgeSession.bridge.setIsOperational(false);
					bridgeSession.hasFailed = true;
				}

				// Check if the conference is in progress
				if (!StringUtils.isNullOrEmpty(bridgeSession.colibriConference.getConferenceId())) {
	...
	protected abstract ColibriConferenceIQ doAllocateChannels(List<ContentPacketExtension> offer)
	 ...
	 *
	 * @param offer           the list which contains Jingle content to be included
	 *                        in the offer.
	 * @param colibriChannels the {@link ColibriConferenceIQ} which represents the
	 *                        channels allocated on a jitsi-videobridge instance for
	 *                        the participant for which the Jingle offer is being
	 *                        prepared.
	 */
	...
	/**
	 * @return the {@link JitsiMeetConferenceImpl.BridgeSession} instance of this
	 *         {@link AbstractChannelAllocator}.
	 */
