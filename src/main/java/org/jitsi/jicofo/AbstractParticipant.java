...
public abstract class AbstractParticipant {
	 ...
	 */
	private final static Logger classLogger = Logger.getLogger(AbstractParticipant.class);
	...
	/**
	 * sources received from other peers scheduled for later addition, because of
	 * the Jingle session not being ready at the point when sources appeared in the
	 * conference.
	 */
	...
	/**
	 * sources received from other peers scheduled for later removal, because of the
	 * Jingle session not being ready at the point when sources appeared in the
	 * conference. FIXME: do we need that since these were never added ? - check
	 */
	 ...
	 * 
	 * @return a <tt>Map<String,RtpDescriptionPacketExtension></tt> which maps the
	 *         RTP descriptions to the corresponding Colibri content names.
	 */
	...
	/**
	 * The {@link AbstractChannelAllocator}, if any, which is currently allocating
	...
	/**
	 * Extracts and stores RTP description for each content type from given Jingle
	 * contents.
	 * 
	...
	/**
	 * Returns <tt>true</tt> if this peer has any not synchronized sources scheduled
	 * for addition.
	 */
	...
	/**
	 * Schedules sources received from other peer for future 'source-remove' update.
	 *
	 * @param sourceMap the media source map that contains sources for future
	 *                  updates.
	 */
	...
	/**
	 * Returns {@link ColibriConferenceIQ} that describes Colibri channels allocated
	...
	/**
	 * Returns the map of source groups that are waiting for being removed from peer
	 * session.
	 */
	 ...
	 * 
	 * @param groupsToRemove the map of source groups that will be removed from this
	 *                       participant media state description.
	...
	/**
	 * Replaces the {@link AbstractChannelAllocator}, which is currently allocating
	 * channels for this participant (if any) with the specified channel allocator
	 * (if any).
	 * 
	 * @param channelAllocator the channel allocator to set, or {@code null} to
	 *                         clear it.
	 */
	public void setChannelAllocator(AbstractChannelAllocator channelAllocator) {
	 ...
	 *                         completed its task and its thread is about to
	 *                         terminate.
	 */
	void channelAllocatorCompleted(AbstractChannelAllocator channelAllocator) {
		synchronized (channelAllocatorSyncRoot) {
	...
	public void addSourcesAndGroups(MediaSourceMap addedSources, MediaSourceGroupMap addedGroups) {
		this.sources.add(addedSources);
		this.sourceGroups.add(addedGroups);
	}

	/**
	 * @return {@code true} if the session with this participant has already been
	 *         established. Before the session is established, we are unable to
