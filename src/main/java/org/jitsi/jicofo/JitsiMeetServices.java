 ...
 * @author Pawel Domas
 */
public class JitsiMeetServices extends EventHandlerActivator {
	/**
	 * The logger
	 */
	 ...
	 * jitsi-videobridge.
	 */
	public static final String[] VIDEOBRIDGE_FEATURES = new String[] { ColibriConferenceIQ.NAMESPACE,
	 ...
	 * server.
	 */
	private static final String[] MUC_FEATURES = { "http://jabber.org/protocol/muc" };

	/**
	 * Features advertised by SIP gateway component.
	 */
	private static final String[] SIP_GW_FEATURES = new String[] { "http://jitsi.org/protocol/jigasi",
			"urn:xmpp:rayo:0" };
	...
	/**
	 * <tt>Version</tt> IQ instance holding detected XMPP server's version (if any).
	 */
	 ...
	 *                                XMPP connection.
	 * @param jicofoUserDomain        the name of the XMPP domain to which Jicofo
	 *                                user
	 */
	public JitsiMeetServices(ProtocolProviderHandler protocolProviderHandler, DomainBareJid jicofoUserDomain) {
		super(new String[] { BridgeEvent.HEALTH_CHECK_FAILED });

		Objects.requireNonNull(protocolProviderHandler, "protocolProviderHandler");

		OperationSetSubscription subscriptionOpSet = protocolProviderHandler
	 ...
	 */
	void newNodeDiscovered(Jid node, List<String> features, Version version) {
		if (isJitsiVideobridge(features)) {
			newBridgeDiscovered(node, version);
		} else if (sipGateway == null && DiscoveryUtil.checkFeatureSupport(SIP_GW_FEATURES, features)) {
			logger.info("Discovered SIP gateway: " + node);

			setSipGateway(node);
		} else if (mucService == null && DiscoveryUtil.checkFeatureSupport(MUC_FEATURES, features)) {
		...
		/*
		 * FIXME: pub-sub service auto-detect ? else if
		 * (capsOpSet.hasFeatureSupport(item, PUBSUB_FEATURES)) { // Potential PUBSUB
		 * service logger.info("Potential PUBSUB service:" + item); List<String>
		 * subItems = capsOpSet.getItems(item); for (String subItem: subItems) {
		 * logger.info("Subnode " + subItem + " of " + item);
		 * capsOpSet.hasFeatureSupport( item, subItem, VIDEOBRIDGE_FEATURES); } }
		 */
	}
	...
	public JibriDetector getSipJibriDetector() {
		return breweryDetectors.stream().filter(d -> d instanceof JibriDetector).map(d -> ((JibriDetector) d))
				.filter(JibriDetector::isSip).findFirst().orElse(null);
	}

	/**
	 * Returns {@link JibriDetector} instance that manages Jibri pool used by this
	 * Jicofo process or <tt>null</tt> if unavailable in the current session.
	 */
	public JibriDetector getJibriDetector() {
		return breweryDetectors.stream().filter(d -> d instanceof JibriDetector).map(d -> ((JibriDetector) d))
				.filter(d -> !d.isSip()).findFirst().orElse(null);
	}

	/**
	 * Returns {@link JigasiDetector} instance that manages Jigasi pool used by this
	 * Jicofo process or <tt>null</tt> if unavailable in the current session.
	 */
	public JigasiDetector getJigasiDetector() {
		return breweryDetectors.stream().filter(d -> d instanceof JigasiDetector).map(d -> ((JigasiDetector) d))
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		if (!StringUtils.isNullOrEmpty(bridgeBreweryName)) {
			BridgeMucDetector bridgeMucDetector = new BridgeMucDetector(protocolProvider, bridgeBreweryName,
					bridgeSelector);
