...
public class AuthenticationSession {
	/**
	 * Machine unique identifier used to associate session with particular user's
	 * machine, so that single user can login from different locations at the same
	 * time.
	 */
	...
	/**
	 * Timestamp in millis used to track the moment of last activity on session
	 * instance, so that it can eventually expire after exceeding time limit of
	 * inactivity.
	 */
	private long activityTimestamp = System.currentTimeMillis();

	/**
	 * User's jabber ID recently used with this session. Used to bind anonymous JID
	 * to {@link #userIdentity}.
	 */
	 ...
	 * @param userIdentity user's identity in the scope of authentication system,
	 *                     usually login name or email address.
	 ...
	 * 
	 * @param userJabberId the Jabber ID of the user that will be associated with
	...
	/**
	 * Returns machine unique identifier which identifies owner machine of this
