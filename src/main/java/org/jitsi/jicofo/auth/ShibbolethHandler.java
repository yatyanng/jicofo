 ...
 * user's identity provided by Shibboleth SP. <br/>
 * <br/>
 * See for more info:
 ...
 * @author Pawel Domas
 */
class ShibbolethHandler extends AbstractHandler {
	/**
	 * The logger instance used by Shibboleth handler.
	 */
	private static final Logger logger = Logger.getLogger(ShibbolethHandler.class);
	...
	private void dumpRequestInfo(HttpServletRequest request) {
		...
		while (attributes.hasMoreElements()) {
			...
			logger.debug(attributeName + ": " + request.getAttribute(attributeName));
		}
	}
	...
	private Map<String, String> createPropertiesMap(HttpServletRequest request) {
		...
		while (attributes.hasMoreElements()) {
			String attributeName = attributes.nextElement();
			propertiesMap.put(attributeName, String.valueOf(request.getAttribute(attributeName)));
		}
	 ...
	 *
	 * @param request <tt>HttpServletRequest</tt> instance used to obtain Shibboleth
	 *                attributes.
	...
	private void doHandle(String target, Request baseRequest, HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException {
		...
		if (request.getParameter("room") == null) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing mandatory parameter 'room'");
			return;
		}

		EntityBareJid roomName = JidCreate.entityBareFrom(request.getParameter("room"));

		String machineUID = request.getParameter("machineUID");
		if (StringUtils.isNullOrEmpty(machineUID)) {
			response.sendError(HttpServletResponse.SC_BAD_REQUEST, "Missing mandatory parameter 'machineUID'");
			return;
		}
		...
		if (!close) {
			responseWriter.println("<h2>You should be redirected back to the conference soon..." + "<h2/>");
		}
		...
		if (close) {
			...
			script += " window.location.href='../" + roomName.getLocalpart() + "';\n";
		}
