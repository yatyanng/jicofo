...
public abstract class AbstractAuthAuthority implements AuthenticationAuthority, FocusManager.FocusAllocationListener {
	 ...
	 * @return the first {@link AuthenticationSession} that matches given predicate
	 *         or <tt>null</tt>.
	 */
	protected AuthenticationSession findSession(Predicate<AuthenticationSession> selector) {
		ArrayList<AuthenticationSession> sessions = new ArrayList<>(authenticationSessions.values());
	...
	protected AuthenticationSession findSessionForIdentity(String machineUID, String authIdentity) {
		...
		return findSession(session -> session.getUserIdentity().equals(authIdentity)
	...
	protected AuthenticationSession findSessionForJabberId(Jid jabberId) {
		...
		return findSession(session -> jabberId.equals(session.getUserJabberId()));
	}
