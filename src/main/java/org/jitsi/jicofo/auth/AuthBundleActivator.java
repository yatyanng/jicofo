 ...
 * @author Pawel Domas
 */
public class AuthBundleActivator extends AbstractJettyBundleActivator {
	/**
	 * The prefix of the names of {@code ConfigurationService} and/or {@code System}
	 * properties defined by {@code AuthBundleActivator}.
	 */
	 ...
	 * pattern of authentication URL. See {@link ShibbolethAuthAuthority} for more
	 * information.
	 */
	 ...
	 * conference ends.
	 */
	public static final String DISABLE_AUTOLOGIN_PNAME = AUTH_PNAME + ".DISABLE_AUTOLOGIN";

	/**
	 * Name of configuration property that controls authentication session lifetime.
	 */
	private final static String AUTHENTICATION_LIFETIME_PNAME = "org.jitsi.jicofo.auth.AUTH_LIFETIME";

	/**
	 * Default lifetime of authentication session(24H).
	 */
	private final static long DEFAULT_AUTHENTICATION_LIFETIME = 24 * 60 * 60 * 1000;
	...
	protected Handler initializeHandlerList(BundleContext bundleContext, Server server) throws Exception {
		...
		if (authAuthority instanceof ShibbolethAuthAuthority) {
			ShibbolethAuthAuthority shibbolethAuthAuthority = (ShibbolethAuthAuthority) authAuthority;
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		long authenticationLifetime = cfg.getLong(AUTHENTICATION_LIFETIME_PNAME, DEFAULT_AUTHENTICATION_LIFETIME);
		boolean disableAutoLogin = cfg.getBoolean(DISABLE_AUTOLOGIN_PNAME, false);
		...
		if (!StringUtils.isNullOrEmpty(loginUrl)) {
			...
			if (loginUrl.toUpperCase().startsWith("XMPP:")) {
				authAuthority = new XMPPDomainAuthAuthority(disableAutoLogin, authenticationLifetime,
						JidCreate.domainBareFrom(loginUrl.substring(5)));
			} else if (loginUrl.toUpperCase().startsWith("EXT_JWT:")) {
				authAuthority = new ExternalJWTAuthority(JidCreate.domainBareFrom(loginUrl.substring(8)));
			} else {
				String logoutUrl = cfg.getString(LOGOUT_URL_PNAME);

				authAuthority = new ShibbolethAuthAuthority(disableAutoLogin, authenticationLifetime, loginUrl,
						logoutUrl);
			}
		}
		...
		if (authAuthority != null) {
			...
			authAuthorityServiceRegistration = bundleContext.registerService(AuthenticationAuthority.class,
					authAuthority, null);
