 ...
 * @author Boris Grozev
 */
public class BridgeSelector implements SubscriptionListener, EventHandler {
	/**
	 * The logger.
	 */
	...
	/**
	 * The name of the property which controls the {@link BridgeSelectionStrategy}
	 * to be used by this {@link BridgeSelector}.
	 */
	public static final String BRIDGE_SELECTION_STRATEGY_PNAME = "org.jitsi.jicofo.BridgeSelector.BRIDGE_SELECTION_STRATEGY";
	 ...
	 * pub-sub node name. If multiple mapping are to be appended then ';' must be
	 * used to separate each mapping.
	 *
	 ...
	 * that offer PubSub feature is selected. Then this selector class subscribes
	 * for all mapped PubSub nodes on that service for notifications.
	 *
	 ...
	 */
	public static final String BRIDGE_FAILURE_RESET_THRESHOLD_PNAME = "org.jitsi.focus.BRIDGE_FAILURE_RESET_THRESHOLD";
	...
	/**
	 * The amount of time we will wait after bridge instance failure before it will
	 * get another chance.
	 */
	...
	public BridgeSelector(OperationSetSubscription subscriptionOpSet) {
		this.subscriptionOpSet = Objects.requireNonNull(subscriptionOpSet, "subscriptionOpSet");

		bridgeSelectionStrategy = Objects.requireNonNull(createBridgeSelectionStrategy());
	...
	private BridgeSelectionStrategy createBridgeSelectionStrategy() {
		...
		if (config != null) {
			String clazzName = config.getString(BRIDGE_SELECTION_STRATEGY_PNAME);
			if (clazzName != null) {
				clazzName = BridgeSelector.class.getCanonicalName() + "$" + clazzName;
				try {
	...
	/**
	 * Adds next Jitsi Videobridge XMPP address to be observed by this selected and
	 * taken into account in best bridge selection process. If a bridge with the
	 * given JID already exists, it is returned and a new instance is not created.
	 *
	 * @param bridgeJid the JID of videobridge to be added to this selector's set of
	 *                  videobridges.
	 ...
	 * @return the {@link Bridge} for the bridge with the provided JID.
	 */
	synchronized public Bridge addJvbAddress(Jid bridgeJid, Version version) {
		...
		if (pubSubNode != null) {
			logger.info("Subscribing to pub-sub notifications to " + pubSubNode + " for " + bridgeJid);
	...
	/**
	 * Removes Jitsi Videobridge XMPP address from the list videobridge instances
	...
	synchronized public void removeJvbAddress(Jid bridgeJid) {
		...
		String pubSubNode = findNodeForBridge(bridgeJid);
		if (pubSubNode != null) {
			logger.info("Removing PubSub subscription to " + pubSubNode + " for " + bridgeJid);
	...
	/**
	 * Selects a bridge to be used for a specific new {@link Participant} of a
	 * specific {@link JitsiMeetConference}.
	 *
	 ...
	 * @param participantRegion the region of the participant for which a bridge is
	 *                          to be selected.
	 */
	synchronized public Bridge selectBridge(JitsiMeetConference conference, String participantRegion,
			boolean allowMultiBridge) {
		List<Bridge> bridges = getPrioritizedBridgesList().stream().filter(Bridge::isOperational)
				.collect(Collectors.toList());
		return bridgeSelectionStrategy.select(bridges, conference, participantRegion, allowMultiBridge);
	}
	 ...
	 * @return the selected bridge, represented by its {@link Bridge}.
	 */
	public Bridge selectBridge(JitsiMeetConference conference) {
	 ...
	 *
	 * @return videobridge JID for given pub-sub node or <tt>null</tt> if no mapping
	 *         found.
	 */
	 ...
	 *
	 * @param bridgeJid the JID of videobridge to be matched with pub-sub node name.
	 *
	 * @return name of pub-sub node mapped for given videobridge JID.
	 */
	private synchronized String findNodeForBridge(Jid bridgeJid) {
		for (Map.Entry<String, Jid> psNodeToBridge : pubSubToBridge.entrySet()) {
	...
	/**
	 * Method called by
	 * {@link org.jitsi.jicofo.ComponentsDiscovery .ThroughPubSubDiscovery} whenever
	 * we receive stats update on shared PubSub node used to discover bridges.
	 * 
	 ...
	 */
	@Override
	synchronized public void onSubscriptionUpdate(String node, String itemId, ExtensionElement payload) {
		...
		if (bridge == null) {
			...
			try {
				...
				logger.warn("Received PubSub update for unknown bridge: " + itemId + " node: "
	...
	/**
	 * Sets the amount of time we will wait after bridge failure before it will get
	 * another chance.
	 *
	 * @param failureResetThreshold the amount of time in millis.
	 *
	 * @throws IllegalArgumentException if given threshold value is equal or less
	 *                                  than zero.
	 */
	public void setFailureResetThreshold(long failureResetThreshold) {
		if (failureResetThreshold <= 0) {
			throw new IllegalArgumentException("Bridge failure reset threshold"
					+ " must be greater than 0, given value: " + failureResetThreshold);
		}
	...
	public void init() {
		...
		setFailureResetThreshold(config.getLong(BRIDGE_FAILURE_RESET_THRESHOLD_PNAME, DEFAULT_FAILURE_RESET_THRESHOLD));
		...
		this.handlerRegistration = EventUtil.registerEventHandler(FocusBundleActivator.bundleContext,
				new String[] { BridgeEvent.VIDEOSTREAMS_CHANGED }, this);
	}

	/**
	 * Finds the version of the videobridge identified by given <tt>bridgeJid</tt>.
	 *
	 * @param bridgeJid the XMPP address of the videobridge for which we want to
	 *                  obtain the version.
	 *
	 * @return {@link Version} instance which holds the details about JVB version or
	...
	private static abstract class BridgeSelectionStrategy {
		 ...
		 */
		private Bridge select(List<Bridge> bridges, JitsiMeetConference conference, String participantRegion,
				boolean allowMultiBridge) {
			List<Bridge> conferenceBridges = conference == null ? new LinkedList<>() : conference.getBridges();
			if (conferenceBridges.isEmpty()) {
				Bridge bridge = selectInitial(bridges, conference, participantRegion);
				if (logger.isDebugEnabled()) {
					logger.debug("Selected initial bridge for " + conference + ": " + bridge);
				}
				return bridge;
			} else {
				if (!allowMultiBridge || conferenceBridges.get(0).getRelayId() == null) {
					if (logger.isDebugEnabled()) {
				...
				return doSelect(bridges, conferenceBridges, conference, participantRegion);
			}
		}

		/**
		 * Selects a bridge to be used for a specific {@link JitsiMeetConference} and a
		 * specific {@link Participant}, assuming that no other bridge is used by the
		 * conference (i.e. this is the initial selection of a bridge for the
		 * conference).
		 *
		 * @param bridges           the list of bridges to select from.
		 * @param conference        the conference for which a bridge is to be selected.
		 ...
		 * @return the selected bridge, or {@code null} if no bridge is available.
		 */
		abstract Bridge doSelect(List<Bridge> bridges, List<Bridge> conferenceBridges, JitsiMeetConference conference,
	 ...
	 */
	public static class SingleBridgeSelectionStrategy extends BridgeSelectionStrategy {
		/**
		 * Default constructor.
		 */
		 ...
		 */
		@Override
		public Bridge doSelect(List<Bridge> bridges, List<Bridge> conferenceBridges, JitsiMeetConference conference,
				String participantRegion) {
			if (conferenceBridges.size() != 1) {
				logger.error("Unexpected number of bridges with " + "SingleBridgeSelectionStrategy: "
			...
			if (!bridge.isOperational()) {
				logger.error("The conference already has a bridge, but it is not " + "operational; conference="
						+ conference.getRoomName() + "; bridge=" + bridge);
				return null;
			}
	 ...
	 */
	private static class SplitBridgeSelectionStrategy extends BridgeSelectionStrategy {
		/**
		 * Default constructor.
		 */
		...
		public Bridge doSelect(List<Bridge> bridges, List<Bridge> conferenceBridges, JitsiMeetConference conference,
			...
			for (Bridge bridge : bridges) {
				...
				if (!conferenceBridges.contains(bridge)) {
					logger.info("Selecting a new bridge for " + conference + ": " + bridge);
					return bridge;
				}
			}
			...
			// at random.
			if (!bridges.isEmpty()) {
				return bridges.get(Math.abs(new Random().nextInt()) % bridges.size());
			}
	 ...
	 */
	private static class RegionBasedBridgeSelectionStrategy extends BridgeSelectionStrategy {
		/**
		 * Default constructor.
		 */
		...
		public Bridge doSelect(List<Bridge> bridges, List<Bridge> conferenceBridges, JitsiMeetConference conference,
			...
			List<Bridge> conferenceBridgesInRegion = conferenceBridges.stream()
					.filter(bridge -> participantRegion.equals(bridge.getRegion())).collect(Collectors.toList());
			...
			Bridge bridgeInRegion = bridges.stream().filter(bridge -> participantRegion.equals(bridge.getRegion()))
		 ...
		 * @param selectFrom A list of bridges to select from. Assumed non-null and
		 *                   non-empty.
		 * @return the bridge from {@code selectFrom} which occurs first in
		 *         {@code order}.
		 */
		private Bridge selectFirst(List<Bridge> selectFrom, List<Bridge> order) {
			return order.stream().filter(selectFrom::contains).findFirst().orElse(selectFrom.get(0));
		}
	}
}
