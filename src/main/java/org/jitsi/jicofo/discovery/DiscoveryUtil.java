...
public class DiscoveryUtil {
	...
	/**
	 * A namespace for our custom "lip-sync" feature. Advertised by the clients that
	 * support all of the functionality required for doing the lip-sync properly.
	 */
	 ...
	 */
	public final static String[] VERSION_FEATURES = new String[] { Version.NAMESPACE };

	/**
	 * Gets the list of features supported by participant. If we fail to obtain it
	 * due to network failure default feature list is returned.
	 * 
	 * @param protocolProvider protocol provider service instance that will be used
	 *                         for discovery.
	 * @param address          XMPP address of the participant.
	 */
	public static List<String> discoverParticipantFeatures(ProtocolProviderService protocolProvider,
		...
		if (logger.isDebugEnabled()) {
			StringBuilder sb = new StringBuilder(address).append(", features: ")
	 ...
	 *
	 * @return {@link Version} if given <tt>jid</tt> supports version IQ and if we
	 *         the query was successful or <tt>null</tt> otherwise.
	 */
	static public Version discoverVersion(XmppConnection connection, Jid jid, List<String> features) {
		...
		if (DiscoveryUtil.checkFeatureSupport(VERSION_FEATURES, features)) {
			...
			try {
				...
				logger.error("Failed to discover component version - XMPP disconnected", e);
				return null;
			}
			...
			if (response instanceof Version) {
				...
				logger.error("Failed to discover version, req: " + versionIq.toXML() + ", response: "
	...
	/**
	 * Checks if all of the features given on <tt>reqFeatures</tt> array exist on
	 * declared list of <tt>capabilities</tt>.
	 * 
	 * @param reqFeatures  array of required features to check.
	 * @param capabilities the list of features supported by the client.
	 * @return <tt>true</tt> if all features from <tt>reqFeatures</tt> array exist
	 *         on <tt>capabilities</tt> list.
	 */
	static public boolean checkFeatureSupport(String[] reqFeatures, List<String> capabilities) {
	...
	/**
	 * Returns <tt>true</tt> if <tt>list1</tt> and <tt>list2</tt> contain the same
	 * elements where items order is not relevant.
	 * 
	 * @param list1 the first list of <tt>String</tt> to be compared against the
	 *              second list.
	 * @param list2 the second list of <tt>String</tt> to be compared against the
	 *              first list.
	 */
	static public boolean areTheSame(List<String> list1, List<String> list2) {
		return list1.size() == list2.size() && list2.containsAll(list1) && list1.containsAll(list2);
	}
}
