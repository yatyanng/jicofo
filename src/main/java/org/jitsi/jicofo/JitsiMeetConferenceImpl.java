...
import org.jitsi.impl.protocol.xmpp.extensions.BridgeNotAvailablePacketExt;
import org.jitsi.impl.protocol.xmpp.extensions.ConferenceProperties;
import org.jitsi.impl.protocol.xmpp.extensions.EtherpadPacketExt;
...
import org.jitsi.jicofo.recording.jibri.JibriRecorder;
import org.jitsi.jicofo.recording.jibri.JibriSipGateway;
...
public class JitsiMeetConferenceImpl
	...
	/**
	 * Sets a conference property and sends an updated presence stanza in the MUC.
	 * 
	 ...
	 * @param key            the key of the property.
	 * @param value          the value of the property.
	 * @param updatePresence {@code true} to send an updated presence stanza, and
	 *                       {@code false} to only add the property locally. This is
	 *                       useful to allow updating multiple properties but
	 *                       sending a single presence update.
	 */
	...
	public void disposeConference() {
		logger.info("Disposing the conference for room: " + getRoomName());
		// If the conference is being disposed the timeout is not needed
		// anymore
		cancelSingleParticipantTimeout();
		logger.info("Bridge Sessions of room " + getRoomName() + ": " + bridges);
		synchronized (bridges) {
			for (BridgeSession bridgeSession : bridges) {
	...
	@Override
	public XMPPError onSessionTerminate(JingleSession jingleSession, ReasonPacketExtension reasonPacketExtension) {
		...
		String message = "";
		Reason reason = Reason.GONE;
		if (reasonPacketExtension != null) {
			reason = reasonPacketExtension.getReason();
			message = reasonPacketExtension.getText();
		}
		this.terminateParticipant(participant, reason, message);
		return null;
	}
	...
	class BridgeSession {
		...
		private void dispose() {
			logger.info("Disposing Colibri Conference: " + colibriConference.getConferenceId());
			logger.info("Colibri Conference has failed: " + hasFailed);
			logger.info("Protocol Provider Handler Registered: " + protocolProviderHandler.isRegistered());
		...
		/**
		 * Expires the COLIBRI channels (via {@link #terminate(AbstractParticipant)})
