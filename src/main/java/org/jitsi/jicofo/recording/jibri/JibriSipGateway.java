...
import net.java.sip.communicator.impl.protocol.jabber.extensions.jibri.SipCallState;
 ...
 * @author Pawel Domas
 */
public class JibriSipGateway extends CommonJibriStuff implements JibriSession.Owner {
	/**
	 * The class logger which can be used to override logging level inherited from
	 * {@link JitsiMeetConference}.
	 */
	static private final Logger classLogger = Logger.getLogger(JibriSipGateway.class);

	/**
	 * Map of SIP {@link JibriSession}s mapped per SIP address which identifies a
	 * SIP Jibri session.
	 */
	 ...
	 */
	public JibriSipGateway(JitsiMeetConferenceImpl conference, XmppConnection xmppConnection,
			ScheduledExecutorService scheduledExecutor, JitsiMeetGlobalConfig globalConfig) {
		super(true /* handles SIP Jibri events */, conference, xmppConnection, scheduledExecutor, globalConfig,
	...
	protected IQ handleStartRequest(JibriIq iq) {
		...
		if (!StringUtils.isNullOrEmpty(sipAddress)) {
			...
			JibriSession jibriSession = new JibriSession(this, conference.getRoomName(),
					globalConfig.getJibriPendingTimeout(), connection, scheduledExecutor, jibriDetector, false,
					sipAddress, displayName, null, null, sessionId, null, classLogger);
	 ...
	 * {@link SipCallState} MUC presence extensions to it's presence. One for each
	 * active SIP Jibri session.
	 * 
	 * @param session       the session for which the new status will be set
	 * @param newStatus     the new status
	 * @param failureReason option error for OFF state
	 */
	private void publishJibriSipCallState(JibriSession session, JibriIq.Status newStatus,
			JibriIq.FailureReason failureReason) {
		...
		logger.info("Publishing new jibri-sip-call-state: " + session.getSipAddress() + sipCallState.toXML() + " in: "
