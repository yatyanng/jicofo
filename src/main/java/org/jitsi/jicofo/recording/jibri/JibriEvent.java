 ...
 * @author Pawel Domas
 */
public class JibriEvent extends Event {
	/** Class logger */
	 ...
	 * available or when some of the existing Jibri changes the status between idle
	 * and busy.
	 */
	...
	/**
	 * The key for event property which stores a <tt>boolean</tt> holding Jibri's
	 * idle status.
	 */
	private final static String IS_IDLE_KEY = "jibri.is_idle";

	/**
	 * The key for event property which stores a <tt>boolean</tt> telling whether or
	 * not the Jibri instance associated with the event is a video SIP gateway Jibri
	 * (<tt>true</tt>) or a live streaming Jibri (<tt>false</tt>).
	 */
	 ...
	 * @param isIdle   a <tt>Boolean</tt> with Jibri's idle status or <tt>null</tt>
	 ...
	 */
	static private Dictionary<String, Object> initDictionary(Jid jibriJid, Boolean isIdle, boolean isSIP) {
	 ...
	 *         <tt>jibriJid</tt>.
	 */
	static public JibriEvent newStatusChangedEvent(Jid jibriJid, boolean isIdle, boolean isSIP) {
	...
	private JibriEvent(String topic, Jid jibriJid, Boolean isIdle, boolean isSIP) {
	 ...
	 *
	 * @return a <tt>boolean</tt> value of <tt>true</tt> for idle or <tt>false</tt>
	 *         for busy.
	 *
	 * @throws IllegalStateException if the method is called on a
	 *                               <tt>JibriEvent</tt> which does not support this
	 *                               property.
	 */
	public boolean isIdle() {
		Boolean isIdle = (Boolean) getProperty(IS_IDLE_KEY);
		if (isIdle == null) {
			throw new IllegalStateException("Trying to access 'isIdle' on wrong event type: " + getTopic());
		}
		return isIdle;
	}

	/**
	 * @return <tt>true</tt> if the event is for SIP Jibri or <tt>false</tt> for a
	 *         regular Jibri.
	 */
