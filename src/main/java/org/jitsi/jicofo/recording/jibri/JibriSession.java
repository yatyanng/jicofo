...
public class JibriSession {
	...
	/**
	 * Returns <tt>true</tt> if given <tt>status</tt> indicates that Jibri is in the
	 * middle of starting of the recording process.
	 */
	...
	/**
	 * The JID of the Jibri currently being used by this session or <tt>null</tt>
	 * otherwise.
	 */
	...
	/**
	 * {@link JibriDetector} instance used to select a Jibri which will be used by
	 * this session.
	 */
	...
	/**
	 * The broadcast id of the YouTube broadcast, if available. This is used to
	 * generate and distribute the viewing url of the live stream
	 */
	 ...
	 * 
	 * @param owner              the session owner which will be notified about this
	 ...
	 *                           state, before trying another Jibri instance or
	 *                           failing with an error.
	 * @param connection         the XMPP connection which will be used to
	 *                           send/listen for packets.
	 * @param scheduledExecutor  the executor service which will be used to schedule
	 *                           pending timeout task execution.
	 * @param jibriDetector      the Jibri detector which will be used to select
	 *                           Jibri instance.
	 * @param isSIP              <tt>true</tt> if it's a SIP session or
	 *                           <tt>false</tt> for a regular live streaming Jibri
	 *                           type of session.
	 ...
	 * @param applicationData    a JSON-encoded string containing
	 *                           application-specific data for Jibri
	 * @param logLevelDelegate   logging level delegate which will be used to select
	 *                           logging level for this instance {@link #logger}.
	 */
	JibriSession(JibriSession.Owner owner, EntityBareJid roomName, long pendingTimeout, XmppConnection connection,
			ScheduledExecutorService scheduledExecutor, JibriDetector jibriDetector, boolean isSIP, String sipAddress,
			String displayName, String streamID, String youTubeBroadcastId, String sessionId, String applicationData,
	...
	/**
	 * Starts this session. A new Jibri instance will be selected and start request
	 * will be sent (in non blocking mode).
	 * 
	...
	/**
	 * Accept only XMPP packets which are coming from the Jibri currently used by
	 * this session. {@inheritDoc}
	 */
	...
	/**
	 * @return a string describing this session instance, used for logging purpose
	 */
	 ...
	 * 
	 * @return the recording mode for this session (STREAM, FILE or UNDEFINED in the
	 *         case that this isn't a recording session but actually a SIP session)
	 */
	...
	/**
	 * Sends an IQ to the given Jibri instance and asks it to start recording/SIP
	 * call.
	 */
	private void sendJibriStartIq(final Jid jibriJid) {
		...
		logger.info("Starting Jibri " + jibriJid
				+ (isSIP ? ("for SIP address: " + sipAddress) : (" for stream ID: " + streamID)) + " in room: "
				+ roomName);
	...
	/**
	 * Method schedules/reschedules {@link PendingStatusTimeout} which will clear
	 * recording state after {@link JitsiMeetGlobalConfig#getJibriPendingTimeout()}.
	 */
	private void reschedulePendingTimeout() {
		...
		if (pendingTimeout > 0) {
			pendingTimeoutTask = scheduledExecutor.schedule(new PendingStatusTimeout(), pendingTimeout,
					TimeUnit.SECONDS);
		}
	}
	...
	private void setJibriStatus(JibriIq.Status newStatus, JibriIq.FailureReason failureReason) {
		...
		if (pendingTimeoutTask != null && !JibriIq.Status.PENDING.equals(newStatus)) {
			pendingTimeoutTask.cancel(false);
	...
	/**
	 * @return SIP address received from Jitsi Meet, which is used for SIP gateway
	 *         session (makes sense only for SIP sessions).
	 */
	...
	/**
	 * Get the unique ID for this session. This is used to uniquely identify a Jibri
	 * session instance, even of the same type (meaning, for example, that two file
	 * recordings would have different session IDs). It will be passed to Jibri and
	 * Jibri will put the session ID in its presence, so the Jibri user for a
	 * particular session can be identified by the clients.
	 * 
	...
	private class JibriEventHandler extends EventHandlerActivator {
		...
		private JibriEventHandler() {
			super(new String[] { JibriEvent.STATUS_CHANGED, JibriEvent.WENT_OFFLINE });
		}
		...
		public void handleEvent(Event event) {
			...
			synchronized (JibriSession.this) {
				if (JibriEvent.WENT_OFFLINE.equals(topic) && jibriJid.equals(currentJibriJid)) {
					logger.error(nickname() + " went offline: " + jibriJid + " for room: " + roomName);
	...
	/**
	 * Task scheduled after we have received RESULT response from Jibri and entered
	 * PENDING state. Will abort the recording if we do not transit to ON state,
	 * after {@link JitsiMeetGlobalConfig#getJibriPendingTimeout()} limit is
	 * exceeded.
	 */
	...
	/**
	 * Interface instance passed to {@link JibriSession} constructor which specifies
	 * the session owner which will be notified about any status changes.
	 */
	public interface Owner {
		 ...
		 */
		void onSessionStateChanged(JibriSession jibriSession, JibriIq.Status newStatus,
