...
/**
 * Handles conference recording through Jibri. Waits for updates from
 * {@link JibriDetector} about recorder instance availability and publishes that
 * information in Jicofo's MUC presence. Handles incoming Jibri IQs coming from
 * conference moderator to start/stop the recording.
 *
 * @author Pawel Domas
 * @author Sam Whited
 */
public class JibriRecorder extends CommonJibriStuff implements JibriSession.Owner {
	/**
	 * The class logger which can be used to override logging level inherited from
	 * {@link JitsiMeetConference}.
	 */
	static private final Logger classLogger = Logger.getLogger(JibriRecorder.class);
	 ...
	 */
	public JibriRecorder(JitsiMeetConferenceImpl conference, XmppConnection connection,
			ScheduledExecutorService scheduledExecutor, JitsiMeetGlobalConfig globalConfig) {
		super(false /* deals with non SIP Jibri events */, conference, connection, scheduledExecutor, globalConfig,
	...
	/**
	 * Accepts only {@link JibriIq} without SIP address. {@inheritDoc}
	 */
	@Override
	...
	/**
	 * Returns the current recording session if any. {@inheritDoc}
	 */
	@Override
	...
	/**
	 * Starts new session for given iq. It is assumed that {@link CommonJibriStuff}
	 * has checked that there is no recording session currently active.
	 * {@inheritDoc}
	 */
	@Override
	protected IQ handleStartRequest(JibriIq iq) {
		...
		if ((!emptyStreamId && (recordingMode.equals(RecordingMode.STREAM))
				|| recordingMode.equals(RecordingMode.UNDEFINED))
				|| (emptyStreamId && recordingMode.equals(RecordingMode.FILE))) {
			String sessionId = generateSessionId();
			jibriSession = new JibriSession(this, conference.getRoomName(), globalConfig.getJibriPendingTimeout(),
					connection, scheduledExecutor, jibriDetector, false, null, displayName, streamID,
					youTubeBroadcastId, sessionId, applicationData, classLogger);
			...
			return IQ.createErrorResponse(iq, XMPPError.from(XMPPError.Condition.bad_request,
			...
			return IQ.createErrorResponse(iq,
					XMPPError.from(XMPPError.Condition.bad_request, "Stream ID is empty or undefined"));
		} else {
	 ...
	 */
	@Override
	public void onSessionStateChanged(JibriSession jibriSession, JibriIq.Status newStatus,
	...
	private void publishJibriRecordingStatus(JibriIq.Status newStatus, JibriIq.FailureReason failureReason) {
		logger.info("Got jibri status " + newStatus + " and failure " + failureReason);
		if (jibriSession == null) {
			// It's possible back-to-back 'stop' requests could be received, and while
			// processing
		...
		logger.info("Publishing new jibri-recording-status: " + recordingStatus.toXML() + " in: "
