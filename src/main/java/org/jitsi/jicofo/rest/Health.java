...
public class Health {
	...
	/**
	 * The name of the parameter that triggers known bridges listing in health check
	 * response;
	 */
	 ...
	 *
	 * @param focusManager the {@code FocusManager} to check the health (status) of
	 * @throws Exception if an error occurs while checking the health (status) of
	 *                   {@code focusManager} or the check determines that
	 *                   {@code focusManager} is not healthy
	 */
	private static void check(FocusManager focusManager) throws Exception {
		...
		if (mucService == null) {
			logger.error("No MUC service found on XMPP domain or Jicofo has not"
		...
		do {
			roomName = JidCreate.entityBareFrom(generateRoomName(), mucService.asDomainBareJid());
	...
	private static Localpart generateRoomName() {
		try {
			return Localpart.from(
					Health.class.getName() + "-" + Long.toHexString(System.currentTimeMillis() + RANDOM.nextLong()));
	 ...
	 * {@link FocusManager}. The method is synchronized so anything other than the
	 * health check itself (which is cached) needs to return very quickly.
	 *
	 * @param focusManager the {@code FocusManager} to get the health (status) of in
	 *                     the form of a JSON representation
	 ...
	 */
	static synchronized void getJSON(FocusManager focusManager, Request baseRequest, HttpServletRequest request,
		...
		try {
			...
			if (Boolean.parseBoolean(listJvbParam)) {
				...
				if (activeJVBs.isEmpty()) {
					logger.error("The health check failed - 0 active JVB instances !");
			...
			if (System.currentTimeMillis() - cachedStatusTimestamp < STATUS_CACHE_INTERVAL && cachedStatus > 0) {
			...
			if (ex instanceof IOException)
				...
				status = cacheStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
		}
	...
	/**
	 * Returns a list of currently healthy JVBs known to Jicofo and kept alive by
	 * our {@link org.jitsi.jicofo.JvbDoctor}.
	 * 
	...
	private static List<Jid> listBridges(FocusManager focusManager) {
		JitsiMeetServices services = Objects.requireNonNull(focusManager.getJitsiMeetServices(), "services");

		BridgeSelector bridgeSelector = Objects.requireNonNull(services.getBridgeSelector(), "bridgeSelector");
	 ...
	 */
	private static class HealthChecksMonitor extends TimerTask {
		/**
		...
		public void run() {
			...
			for (ThreadInfo threadInfo : threadInfos) {
				...
				for (int i = 0; i < stackTraceElements.length; i++) {
					...
					if (i == 0 && threadInfo.getLockInfo() != null) {
						...
						if (ts == Thread.State.BLOCKED || ts == Thread.State.WAITING
								|| ts == Thread.State.TIMED_WAITING) {
							dbg.append("\t-  " + ts + " on " + threadInfo.getLockInfo());
							dbg.append('\n');
						}
					}
			...
			logger.error(
					"Health check took " + (System.currentTimeMillis() - this.startedAt) + " ms. \n" + dbg.toString());
		}

		/**
		 * Starts the monitor. Schedules execution in separate thread after some
		 * interval, if monitor is not stopped and it executes we consider the health
		 * check took too much time.
		 */
		public void start() {
