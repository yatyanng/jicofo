...
public class FocusAccountFactory {
	...
	/**
	 * Creates focus XMPP account properties configured for given <tt>domain</tt>
	 * which uses anonymous login method.
	 *
	 * @param serverAddress XMPP server address.
	 * @param domain        name of the XMPP domain on which the focus will
	 *                      register.
	 * @param userName      user name used by the focus user.
	 *
	 * @return the map of new focus account properties for given domain.
	 */
	public static Map<String, String> createFocusAccountProperties(String serverAddress, DomainBareJid domain,
			Resourcepart userName) {
		HashMap<String, String> properties = new HashMap<String, String>();

		Resourcepart resource = null;
		try {
			resource = Resourcepart.from(userName.toString() + System.nanoTime());
		...
		properties.put(ProtocolProviderFactory.DEFAULT_SIPZRTP_ATTRIBUTE, "false");
		...
		properties.put(ProtocolProviderFactory.AUTO_DISCOVER_JINGLE_NODES, "false");
	...
	/**
	 * Creates focus XMPP account properties configured for given <tt>domain</tt>
	 * which login as authenticated admin user (we expect it to be admin or
	 * otherwise focus will refuse to join the room).
	 *
	 * @param serverAddress XMPP server address.
	 * @param domain        name of the XMPP domain on which the focus will
	 *                      register.
	 ...
	 */
	public static Map<String, String> createFocusAccountProperties(String serverAddress, DomainBareJid domain,
			Resourcepart userName, String password) {
		Map<String, String> properties = createFocusAccountProperties(serverAddress, domain, userName);

		properties.put(ProtocolProviderFactory.AUTHORIZATION_NAME, userName.toString());

		/*
		 * String pass = new String(Base64.encode(password.getBytes()));
		 * properties.put(ProtocolProviderFactory.PASSWORD, pass);
		 */
