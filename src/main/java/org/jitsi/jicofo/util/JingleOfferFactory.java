...
public class JingleOfferFactory {
	 ...
	 * session-invite.
	 */
	public static final String VP9_RTX_PT_PNAME = "org.jitsi.jicofo.VP9_RTX_PT";
	 ...
	 * session-invite.
	 */
	public static final String H264_RTX_PT_PNAME = "org.jitsi.jicofo.H264_RTX_PT";

	/**
	 * The name of the property which enables the inclusion of the framemarking RTP
	 * header extension in the offer.
	 */
	public static final String ENABLE_FRAMEMARKING_PNAME = "org.jitsi.jicofo.ENABLE_FRAMEMARKING";

	/**
	 * The name of the property which enables the inclusion of the AST RTP header
	 * extension in the offer.
	 */
	public static final String ENABLE_AST_PNAME = "org.jitsi.jicofo.ENABLE_AST";

	/**
	 * The name of the property which enables the inclusion of the TOF RTP header
	 * extension in the offer.
	 */
	...
	/**
	 * Whether to enable the framemarking RTP header extension in created offers.
	 */
	...
	public JingleOfferFactory(ConfigurationService cfg) {
		...
		enableFrameMarking = cfg != null && cfg.getBoolean(ENABLE_FRAMEMARKING_PNAME, false);
	...
	/**
	 * Creates a {@link ContentPacketExtension} for the audio media type that will
	 * be included in initial conference offer.
	 *
	 * @param disableIce pass <tt>true</tt> if RAW transport instead of ICE should
	 ...
	 */
	public ContentPacketExtension createAudioContent(boolean disableIce, boolean useDtls, boolean stereo,
			boolean enableRemb, boolean enableTcc) {
		ContentPacketExtension content = createContentPacketExtension(MediaType.AUDIO, disableIce, useDtls);
	...
	/**
	 * Creates a {@link ContentPacketExtension} for the data media type that will be
	 ...
	 *         used in initial conference offer.
	 */
	public ContentPacketExtension createDataContent(boolean disableIce, boolean useDtls) {
		ContentPacketExtension content = createContentPacketExtension(MediaType.DATA, disableIce, useDtls);
	...
	/**
	 * Creates a {@link ContentPacketExtension} for the video media type that will
	 * be included in initial conference offer.
	 *
	 * @param disableIce   pass <tt>true</tt> if RAW transport instead of ICE should
	 *                     be indicated in the offer.
	 ...
	 * @param minBitrate   the value to set to the "x-google-min-bitrate" fmtp line
	 *                     for video, or -1 to not add such a line.
	 ...
	 *         used in initial conference offer.
	 */
	public ContentPacketExtension createVideoContent(boolean disableIce, boolean useDtls, boolean useRtx,
			boolean enableRemb, boolean enableTcc, int minBitrate, int startBitrate) {
		ContentPacketExtension videoContentPe = createContentPacketExtension(MediaType.VIDEO, disableIce, useDtls);

		addVideoToContent(videoContentPe, useRtx, enableRemb, enableTcc, minBitrate, startBitrate);

		return videoContentPe;
	}

	/**
	 * Creates <tt>ContentPacketExtension</tt> initialized with type of the media
	 * and basic transport information based on given parameters. The creator
	 * attribute is set to "initiator" and "senders" to "both".
	 *
	 ...
	 *                   transport(will have effect only if <tt>disableIce</tt></tt>
	 *                   is <tt>false</tt>)
	 *
	 * @return new, parametrized instance of <tt>ContentPacketExtension</tt>.
	 */
	private static ContentPacketExtension createContentPacketExtension(MediaType mediaType, boolean disableIce,
			boolean useDtls) {
		ContentPacketExtension content = new ContentPacketExtension(ContentPacketExtension.CreatorEnum.initiator,
	 ...
	 */
	private void addVideoToContent(ContentPacketExtension content, boolean useRtx, boolean enableRemb,
			boolean enableTcc, int minBitrate, int startBitrate) {
		...
		PayloadTypePacketExtension vp8 = addPayloadTypeExtension(rtpDesc, VP8_PT, Constants.VP8, 90000);
		...
		if (minBitrate != -1) {
			addParameterExtension(vp8, "x-google-min-bitrate", String.valueOf(minBitrate));
		}

		if (startBitrate != -1) {
			addParameterExtension(vp8, "x-google-start-bitrate", String.valueOf(startBitrate));
		}
		...
		if (minBitrate != -1) {
			addParameterExtension(h264, "x-google-min-bitrate", String.valueOf(minBitrate));
		}

		if (startBitrate != -1) {
			addParameterExtension(h264, "x-google-start-bitrate", String.valueOf(startBitrate));
		}

		addParameterExtension(h264, "profile-level-id", "42e01f;level-asymmetry-allowed=1;packetization-mode=1;");

		// a=rtpmap:101 VP9/90000
		PayloadTypePacketExtension vp9 = addPayloadTypeExtension(rtpDesc, VP9_PT, Constants.VP9, 90000);
		...
		if (enableTcc) {
			...
			vp9.addRtcpFeedbackType(createRtcpFbPacketExtension("transport-cc", null));
		}

		if (minBitrate != -1) {
			addParameterExtension(vp9, "x-google-min-bitrate", String.valueOf(minBitrate));
		}

		if (startBitrate != -1) {
			addParameterExtension(vp9, "x-google-start-bitrate", String.valueOf(startBitrate));
		}

		if (useRtx) {
			// a=rtpmap:96 rtx/90000
			PayloadTypePacketExtension rtx = addPayloadTypeExtension(rtpDesc, VP8_RTX_PT, Constants.RTX, 90000);
			...
			PayloadTypePacketExtension rtxH264 = addPayloadTypeExtension(rtpDesc, H264_RTX_PT, Constants.RTX, 90000);

			// a=fmtp:99 apt=107
			addParameterExtension(rtxH264, "apt", String.valueOf(H264_PT));

			// a=rtpmap:97 rtx/90000
			PayloadTypePacketExtension rtxVP9 = addPayloadTypeExtension(rtpDesc, VP9_RTX_PT, Constants.RTX, 90000);
	 ...
	 */
	private static ParameterPacketExtension addParameterExtension(PayloadTypePacketExtension ptExt, String name,
			String value) {
	 ...
	 */
	private static RtcpFbPacketExtension createRtcpFbPacketExtension(String type, String subtype) {
	 ...
	 */
	private static PayloadTypePacketExtension addPayloadTypeExtension(RtpDescriptionPacketExtension rtpDesc, int id,
			String name, int clockRate) {
	 ...
	 */
	private static void addAudioToContent(ContentPacketExtension content, boolean stereo, boolean enableRemb,
			boolean enableTcc) {
		RtpDescriptionPacketExtension rtpDesc = new RtpDescriptionPacketExtension();
		...
		PayloadTypePacketExtension opus = addPayloadTypeExtension(rtpDesc, 120, Constants.OPUS, 48000);
		opus.setChannels(2);
		...
		addPayloadTypeExtension(rtpDesc, 96, Constants.TELEPHONE_EVENT, 8000);
	 ...
	 * 
	 * @param contents the list of <tt>ContentPacketExtension</tt> describing Jingle
	 *                 offer.
	 * @return <tt>true</tt> if given offer has video content.
	 */
	public static boolean containsVideoContent(List<ContentPacketExtension> contents) {
