...
public class Bridge implements Comparable<Bridge> {
	 ...
	 * video streams. This should match {@code VideobridgeStatistics.VIDEOSTREAMS},
	 * but is defined separately to avoid depending on the {@code jitsi-videobridge}
	 * maven package.
	 */
	private static final String STAT_NAME_VIDEO_STREAMS = "videostreams";

	/**
	 * The name of the stat used by jitsi-videobridge to indicate its region. This
	 * should match {@code VideobridgeStatistics.REGION}, but is defined separately
	 * to avoid depending on the {@code jitsi-videobridge} maven package.
	 */
	private static final String STAT_NAME_REGION = "region";

	/**
	 * The name of the stat used by jitsi-videobridge to indicate its Octo relay ID.
	 * This should match {@code VideobridgeStatistics.RELAY_ID}, but is defined
	 * separately to avoid depending on the {@code jitsi-videobridge} maven package.
	 */
	 ...
	 */
	private static final ColibriStatsExtension EMPTY_STATS = new ColibriStatsExtension();
	 ...
	 * {@link BridgeEvent#VIDEOSTREAMS_CHANGED} in order to estimate video stream
	 * count on the bridge. The value is included in the result returned by
	 * {@link #getEstimatedVideoStreamCount()} if not <tt>null</tt>.
	 *
	 * Is is set back to <tt>null</tt> when new value from the bridge arrives.
	 */
	...
	/**
	 * Stores the {@code operational} status of the bridge, which is {@code true} if
	 * the bridge has been successfully used by the focus to allocate channels. It
	 * is reset to {@code false} when the focus fails to allocate channels, but it
	 * gets another chance when all currently working bridges go down and might
	 * eventually get elevated back to {@code true}.
	 */
	private volatile boolean isOperational = true /* we assume it is operational */;
	...
	/**
	 * Notifies this instance that a new {@link ColibriStatsExtension} was received
	 * for this instance.
	 * 
	 * @param stats the {@link ColibriStatsExtension} instance which was received.
	 */
	void setStats(ColibriStatsExtension stats) {
		...
		setIsOperational(!Boolean.valueOf(stats.getValueAsString(JigasiDetector.STAT_NAME_SHUTDOWN_IN_PROGRESS)));
	}

	Bridge(BridgeSelector bridgeSelector, Jid jid, Version version) {
	...
	/**
	 * @return the relay ID advertised by the bridge, or {@code null} if none was
	 *         advertised.
	 */
	...
	public void setVideoStreamCount(int streamCount) {
		...
		if (videoStreamCountDiff != 0) {
			...
			logger.info("Reset video stream diff on " + this.jid + " video streams: " + this.videoStreamCount
					+ " (estimation error: "
	...
	private void verifyFailureThreshold() {
		...
		if (System.currentTimeMillis() - failureTimestamp > bridgeSelector.getFailureResetThreshold()) {
	...
	public int compareTo(Bridge o) {
		...
		return this.getEstimatedVideoStreamCount() - o.getEstimatedVideoStreamCount();
	}
	...
	void onVideoStreamsChanged(Integer videoStreamCount) {
		...
		logger.info((adding ? "Adding " : "Removing ") + Math.abs(videoStreamCount) + " video streams on " + this.jid
				+ " video streams: " + this.videoStreamCount + " diff: " + videoStreamCountDiff + " (estimated: "
	...
	public String toString() {
		return "[Bridge, jid=" + jid.toString() + ", relayId=" + getRelayId() + ", region=" + getRegion() + "]";
	}
}
