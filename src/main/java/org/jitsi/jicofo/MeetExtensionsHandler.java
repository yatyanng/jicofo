...
import org.jitsi.impl.protocol.xmpp.extensions.StartMutedProvider;
...
public class MeetExtensionsHandler {
	 ...
	 * 
	 * @param focusManager <tt>FocusManager</tt> that will be used by new instance
	 *                     to access active conferences and focus XMPP connection.
	 */
	...
	private class MuteIqHandler extends AbstractIqRequestHandler {
		MuteIqHandler() {
			super(MuteIq.ELEMENT_NAME, MuteIq.NAMESPACE, IQ.Type.set, Mode.sync);
		}
	...
	private class DialIqHandler extends AbstractIqRequestHandler {
		DialIqHandler() {
			super(RayoIqProvider.DialIq.ELEMENT_NAME, RayoIqProvider.NAMESPACE, IQ.Type.set, Mode.sync);
		}
	...
	private IQ handleRayoIQ(RayoIqProvider.DialIq dialIq) {
		...
		if (jigasiJid == null) {
			// Not available
			return IQ.createErrorResponse(dialIq,
		...
		try {
			...
			if (reply == null) {
				return IQ.createErrorResponse(dialIq, XMPPError.getBuilder(XMPPError.Condition.remote_server_timeout));
			}
			...
			return IQ.createErrorResponse(dialIq, XMPPError.getBuilder(XMPPError.Condition.internal_server_error)
