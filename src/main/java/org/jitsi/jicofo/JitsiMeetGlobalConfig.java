...
public class JitsiMeetGlobalConfig {
	 ...
	 */
	private final static String DISABLE_AUTO_OWNER_PNAME = "org.jitsi.jicofo.DISABLE_AUTO_OWNER";
	...
	/**
	 * The name of the config property which specifies how long we're going to wait
	 * for Jibri to start recording from the time it accepted START request
	 */
	private static final String JIBRI_PENDING_TIMEOUT_PROP_NAME = "org.jitsi.jicofo.jibri.PENDING_TIMEOUT";
	 ...
	 * next participant be selected as new owner.
	 */
	private boolean autoOwner = true;

	/**
	 * Tells how many seconds we're going to wait for the Jibri to start recording.
	 * If set to <tt>-1</tt> it means that these timeouts are disabled in the
	 * current session.
	 */
	...
	/**
	 * Tells how long participant's media session will be kept alive once it remains
	 * the only person in the room - which means that nobody is receiving his/her
	 * media. This participant could be timed out immediately as well, but we don't
	 * want to reallocate channels when the other peer is only reloading his/her
	 * page. The value is amount of time measured in milliseconds.
	 */
	 ...
	 *
	 * @param bc the context for which we're going to obtain global config instance.
	 *
	 * @return <tt>JitsiMeetGlobalConfig</tt> if one is currently registered as a
	 *         service in given OSGi context or <tt>null</tt> otherwise.
	 */
	 ...
	 *
	 * @param configService <tt>ConfigService</tt> the configuration service which
	...
	private void init(ConfigurationService configService) {
		...
		this.maxSourcesPerUser = configService.getInt(MAX_SSRC_PER_USER_CONFIG_PNAME, DEFAULT_MAX_SSRC_PER_USER);

		jibriPendingTimeout = configService.getInt(JIBRI_PENDING_TIMEOUT_PROP_NAME, JIBRI_DEFAULT_PENDING_TIMEOUT);
		...
		singleParticipantTimeout = configService.getLong(SINGLE_PARTICIPANT_TIMEOUT_CONFIG_PNAME,
	 ...
	 * current session.
	 *
	 * @return <tt>int</tt> which is the number of seconds we wait for the Jibri to
	 *         start recording after it accepted our request.
	 */
	...
	/**
	 * Indicates whether auto owner feature is active. First participant to join the
	 * room will become conference owner. When the owner leaves the room next
	 * participant will be selected as the new owner.
	 *
	 * @return <tt>true</tt> if the auto-owner feature is enabled or <tt>false</tt>
	 *         otherwise.
	 */
