...
import org.jitsi.impl.protocol.xmpp.extensions.ReservationErrorPacketExt;
 ...
 * that given user is allowed to create the room. <tt>ReservationSystem</tt>
 * itself is responsible for destroying conference when it should expire by
 * calling the method
...
public interface ReservationSystem {
	 ...
	 * @return <tt>Result</tt> with code {@link #RESULT_OK} if conference has been
	 *         created successfully or another code with error details if something
	 *         went wrong. Error details will be returned in XMPP error IQ
	 *         containing {@link ReservationErrorPacketExt}.
	 */
	...
	class Result {
		 ...
		 * 
		 * @param resultCode one of {@link ReservationSystem} RESULT_... constants.
		 */
		 ...
		 * 
		 * @param errorCode    one of {@link ReservationSystem} RESULT_... constants.
		 * @param errorMessage error message which will be displayed to the user.
		 */
		...
		/**
		 * Returns reservation system result code. One of {@link ReservationSystem}
		 * RESULT_... constants.
		 */
		public int getCode() {
			return code;
		...
		/**
		 * Error message string(if any) which will be displayed to the user in case of
		 * error result code.
		 */
		...
		public String toString() {
			return "Result[c=" + code + ", msg=" + errorMessage + "]@" + hashCode();
		}
	}
}
