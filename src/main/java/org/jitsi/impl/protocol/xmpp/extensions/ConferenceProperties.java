...
public class ConferenceProperties extends AbstractPacketExtension {
	...
	/**
	 * The property key used for the conference creation timestamp (in milliseconds
	 * since the Epoch).
	 */
	public static final String KEY_CREATED_MS = "created-ms";

	/**
	 * The property key used for an to signal whether octo is enabled or disabled.
	 * Note that Octo can be enabled but not in use (e.g. when all participants in
	 * the conference are in the same region).
	 */
	 ...
	 * currently used in the conference (a count larger than 1 indicates that Octo
	 * is being used).
	 */
	...
	public static ConferenceProperties clone(ConferenceProperties source) {
		...
		for (ConferenceProperty property : source.getProperties()) {
			destination.addProperty(AbstractPacketExtension.clone(property));
		}
	...
	/**
	 * Removes all {@link ConferenceProperty}-s with a given key from this instance.
	 * 
	...
	private void clear(String key) {
		...
		for (ConferenceProperty property : getChildExtensionsOfType(ConferenceProperty.class)) {
	...
	/**
	 * A {@link org.xmpp.packet.PacketExtension} that represents a key-value pair to
	 * be included in the focus MUC presence.
	 *
	 * @author George Politis
	 */
	public static class ConferenceProperty extends AbstractPacketExtension {
		...
		/**
		 * This should not be used externally, because it leaves the instance without a
		 * "key" and "vale" attribute. It is needed (with public access) because of
		 * {@link AbstractPacketExtension#clone()}, though.
		 */
