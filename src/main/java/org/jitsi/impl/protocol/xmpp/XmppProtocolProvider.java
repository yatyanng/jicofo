 ...
 * @author Pawel Domas
 */
public class XmppProtocolProvider extends AbstractProtocolProviderService {
	/**
	...
	public XmppProtocolProvider(AccountID accountID) {
		...
		try {
			addSupportedOperationSet(OperationSetSubscription.class, new OpSetSubscriptionImpl(this));
	...
	public synchronized void register(SecurityAuthority securityAuthority) throws OperationFailedException {
		...
		try {
			...
			throw new OperationFailedException("Invalid UserID", OperationFailedException.ILLEGAL_ARGUMENT, e);
		}
		...
		XMPPTCPConnectionConfiguration.Builder connConfig = XMPPTCPConnectionConfiguration.builder()
				.setHost(serverAddressUserSetting).setPort(serverPort).setXmppDomain(serviceName);
		...
		ScheduledExecutorService executorService = ServiceUtils.getService(XmppProtocolActivator.bundleContext,
		...
		connectRetry.runRetryingTask(new SimpleRetryTask(0, 5000L, true, getConnectCallable()));
	}
	 ...
	 * detect that there is no chance to get connected any any future retries should
	 * be canceled.
	 */
	synchronized private boolean doConnect() {
		...
		try {
			...
			discoInfoManager = new ScServiceDiscoveryManager(XmppProtocolProvider.this, connection, new String[] {},
					new String[] {}, false);
			...
			EventAdmin eventAdmin = ServiceUtils.getService(XmppProtocolActivator.bundleContext, EventAdmin.class);
	...
	private void notifyConnected() {
		if (!RegistrationState.REGISTERED.equals(registrationState)) {
			...
			fireRegistrationStateChanged(oldState, RegistrationState.REGISTERED,
	...
	private void notifyDisconnected() {
		if (!RegistrationState.UNREGISTERED.equals(registrationState)) {
			...
			fireRegistrationStateChanged(oldState, RegistrationState.UNREGISTERED,
					RegistrationStateChangeEvent.REASON_NOT_SPECIFIED, null);
		}
	}
	...
	public boolean checkFeatureSupport(Jid contactAddress, String[] features) {
		...
		} catch (XMPPException | InterruptedException | NotConnectedException | NoResponseException e) {
	...
	public boolean checkFeatureSupport(Jid node, String subnode, String[] features) {
		for (String feature : features) {
	...
	/**
	 * Listener that just logs that we are currently reconnecting or we failed to
	 * reconnect.
	 */
	static class XmppReConnectionListener implements ReconnectionListener {
		@Override
		public void reconnectingIn(int i) {
	 ...
	 */
	private static class XmppConnectionAdapter implements XmppConnection {
		...
		public void sendIqWithResponseCallback(IQ iq, StanzaListener stanzaListener,
				ExceptionCallback exceptionCallback, long timeout) throws NotConnectedException, InterruptedException {
	...
	private static class DebugLogger implements StanzaFilter, StanzaListener {
