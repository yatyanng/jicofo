    ...
    implements ChatRoom2, PresenceListener
{
    ...
    {
    	logger.debug("Joining As: "+getParentProvider().getAccountID()
    ...
    private void processOwnPresence(Presence presence)
    {
        ...
        if (mucUser != null)
        {
                ...
                && MUCRole.none == role)
            {
                ...
                && MUCAffiliation.owner  == affiliation
                && MUCRole.none == role)
            {
            	logger.debug("About to dispose Colibri Conference of this Jitsi-Meet Conference: " 
            			+conference);
                if (conference != null) 
                {
