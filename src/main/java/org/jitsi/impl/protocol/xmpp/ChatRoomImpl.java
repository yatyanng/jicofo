 ...
 * @author Pawel Domas
 */
public class ChatRoomImpl extends AbstractChatRoom implements ChatRoom2, PresenceListener {
	/**
	 ...
	 */
	private CopyOnWriteArrayList<ChatRoomMemberPropertyChangeListener> propListeners = new CopyOnWriteArrayList<>();
	 ...
	 */
	public ChatRoomImpl(OperationSetMultiUserChatImpl parentChatOperationSet, EntityBareJid roomJid) {
	...
	public void join() throws OperationFailedException {
		logger.debug("Joining As: "
				+ getParentProvider().getAccountID().getAccountPropertyString(ProtocolProviderFactory.DISPLAY_NAME));
		joinAs(getParentProvider().getAccountID().getAccountPropertyString(ProtocolProviderFactory.DISPLAY_NAME));
	}
	...
	public void joinAs(String nickname) throws OperationFailedException {
		try {
			...
			/*
			 * Iterator<FormField> fields = config.getFields(); while (fields.hasNext()) {
			 * FormField field = fields.next(); logger.info("FORM: " + field.toXML()); }
			 */
		...
		} catch (XMPPException | XmppStringprepException | MucAlreadyJoinedException | NotAMucServiceException
				| NoResponseException | NotConnectedException | InterruptedException e) {
	...
	public void leave() {
		...
		/*
		 * HashMap<String, ChatMemberImpl> membersCopy; synchronized (members) {
		 * membersCopy = new HashMap<String, ChatMemberImpl>(members); }
		 * 
		 ...
		 * LocalUserChatRoomPresenceChangeEvent.LOCAL_USER_LEFT, reason,
		 * alternateAddress);
		 */
	...
	/**
	 * Resets cached role instance so that it will be refreshed when
	 * {@link #getUserRole()} is called.
	 */
	 ...
	 * 
	 * @param occupantJid full mucJID of the occupant for whom we want to reset the
	 ...
	 * <tt>ChatRoomLocalUserRoleListener</tt>s that local user's role has been
	 * changed in this <tt>ChatRoom</tt>.
	 *
	...
	public boolean destroy(String reason, String alternateAddress) {
		...
		} catch (XMPPException | XmppStringprepException | InterruptedException | NoResponseException
	...
	private void notifyMemberJoined(ChatMemberImpl member) {
		ChatRoomMemberPresenceChangeEvent event = new ChatRoomMemberPresenceChangeEvent(this, member,
	...
	private void notifyMemberPropertyChanged(ChatMemberImpl member) {
		ChatRoomMemberPropertyChangeEvent event = new ChatRoomMemberPropertyChangeEvent(member, this,
				ChatRoomMemberPropertyChangeEvent.MEMBER_PRESENCE, null, member.getPresence());
	...
	/**
	 * Returns the MUCUser packet extension included in the packet or <tt>null</tt>
	 * if none.
	 *
	...
	public Collection<ExtensionElement> getPresenceExtensions() {
		return lastPresenceSent != null ? new ArrayList<>(lastPresenceSent.getExtensions()) : EMPTY_PRESENCE_LIST;
	}
	...
	/**
	 * Adds a new {@link ChatMemberImpl} with the given JID to {@link #members}. If
	 * a member with the given JID already exists, it returns the existing instance.
	 * 
	...
	/**
	 * Gets the "real" JID of an occupant of this room specified by its occupant
	 * JID.
	 * 
	...
	private void processOwnPresence(Presence presence) {
		...
		if (mucUser != null) {
			...
			logger.debug("Available: " + presence.isAvailable());
			...
			} else if (!presence.isAvailable() && MUCAffiliation.owner == affiliation && MUCRole.none == role) {
				logger.debug("About to dispose Colibri Conference of this Jitsi-Meet Conference: " + conference);
				if (conference != null) {
					conference.disposeConference();
				}
			} else {
	...
	public void processPresence(Presence presence) {
		...
		logger.debug("My Occupant Jid: " + myOccupantJid + ", The presence is from: " + presence.getFrom());
		...
		public void left(EntityFullJid occupantJid) {
			synchronized (members) {
				...
				if (member != null) {
					...
					logger.warn("Member left event for non-existing member: " + occupantJid);
				}
			}
		}
