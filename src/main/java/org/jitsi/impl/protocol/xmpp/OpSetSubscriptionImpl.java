 ...
 * @author Pawel Domas
 */
public class OpSetSubscriptionImpl implements OperationSetSubscription, ItemEventListener {
	/**
	 * The logger used by this instance.
	 */
	private final static Logger logger = Logger.getLogger(OpSetSubscriptionImpl.class);
	...
	/**
	 * The name of configuration property which specifies re-try interval for PubSub
	 * subscribe operation.
	 */
	public static final String PUBUSB_RETRY_INT_PNAME = "org.jitsi.focus.pubsub.RETRY_INTERVAL";
	 ...
	 */
	public OpSetSubscriptionImpl(XmppProtocolProvider parentProvider) throws XmppStringprepException {
		String pubSubAddressString = FocusBundleActivator.getConfigService().getString(PUBSUB_ADDRESS_PNAME);
	...
	private Long getRetryInterval() {
		if (retryInterval == null) {
			ConfigurationService config = ServiceUtils.getService(FocusBundleActivator.bundleContext,
	...
	/**
	 * Lazy initializer for our JID field(it's not available before provider gets
	 * connected).
	 */
	...
	private PubSubManager getManager() {
		return PubSubManager.getInstance(parentProvider.getConnection(), pubSubAddress);
	}
	...
	private boolean isSubscribed(EntityFullJid jid, Node node)
			throws XMPPException.XMPPErrorException, NotConnectedException, InterruptedException, NoResponseException {
		// FIXME: consider using local flag rather than getting the list
		// of subscriptions
		for (org.jivesoftware.smackx.pubsub.Subscription subscription : node.getSubscriptions()) {
	 ...
	 */
	@Override
	public synchronized void subscribe(String node, SubscriptionListener listener) {
	 ...
	 */
	@Override
	public synchronized void unSubscribe(String node, SubscriptionListener listener) {
		synchronized (subscriptionsMap) {
	...
	public List<PayloadItem> getItems(String nodeName) {
		...
		} catch (XMPPException | InterruptedException | NoResponseException | NotConnectedException
				| NotAPubSubNodeException e) {
			logger.error("Failed to fetch PubSub items of: " + nodeName + ", reason: " + e);
		}
		return null;
	}
	...
	/**
	 * Class holds info about PubSub subscription and implement subscribe re-try
	 * logic.
	 */
	class Subscription {
		/**
		 ...
		 * @param listener subscription listener which will be notified about PubSub
		 *                 updates.
		 */
		public Subscription(String node, SubscriptionListener listener) {
			...
			this.retryStrategy = new RetryStrategy(XmppProtocolActivator.bundleContext);
		}
		 ...
		 * @return <tt>false</tt> if there are no more listeners registered to this
		 *         <tt>Subscription</tt>
		 */
		 ...
		 * <tt>PayloadItem</tt>.
		 * 
		 * @param payloadItem new <tt>PayloadItem</tt> published to the PubSub node
		...
		/**
		 * Tries to subscribe to PubSub node notifications and returns <tt>true</> on
		 * success.
		 *
		 * @throws Exception if application specific error occurs
		 */
		synchronized private boolean doSubscribe() throws Exception {
			...
			try {
				...
				logger.error("Failed to subscribe to " + node + " at " + pubSubAddress + " error: " + e);
		...
		synchronized void subscribe() {
			retryStrategy.runRetryingTask(new SimpleRetryTask(0, getRetryInterval(), true, getRetryCallable()));
		}
		...
		synchronized void unSubscribe() {
			...
			if (!parentProvider.isRegistered()) {
				logger.warn("No connection - skipped PubSub unsubscribe for: " + node);
				return;
			}
			...
			try {
				...
				if (isSubscribed(ourJid, pubSubNode)) {
					...
					| NotAPubSubNodeException e) {
				logger.error("An error occurred while trying to unsubscribe from" + " PubSub: " + node + " at "
						+ pubSubAddress + ", reason: " + e);
			}
		}
